...
class Process
{
   var $_POST;
	...
	function __construct() {
	   $this->Obj = new JenisPegawai;
	   $this->_POST = $_REQUEST->AsArray();
	   $this->decId = $_GET['id']->Integer()->Raw();
	   $this->user=Security::Authentication()->GetCurrentUser()->GetUserId();
	   $this->encId = $this->decId;
	   $this->pageView = Dispatcher::Instance()->GetUrl('jenis_pegawai', 'JenisPegawai', 'view', 'html');
	   $this->pageInput = Dispatcher::Instance()->GetUrl('jenis_pegawai', 'JenisPegawai', 'view', 'html');
	...
	function Check ()
   {
	  ...
	  if ((trim($this->_POST['nama']) == '') or ($this->_POST['urut'] == '')){
	  ...
	  if (isset($error))
      {
         ...
         Messenger::Instance()->Send('jenis_pegawai', 'JenisPegawai', 'view', 'html', $msg, Messenger::NextRequest);
   ...
   function Update()
   {
	  ...
	  $data=array('nama'=>$this->_POST['nama'],'tipe'=>$this->_POST['tipe'],'urut'=>$this->_POST['urut'],'userId'=>$this->user,'id'=>$this->decId);
   ...
   function Add()
   {
     $check = $this->Check();
	  ...
	  $data=array('nama'=>$this->_POST['nama'],'tipe'=>$this->_POST['tipe'],'urut'=>$this->_POST['urut']);//print_r($data);exit;
