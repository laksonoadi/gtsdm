...
class ViewMutasiDp3 extends HtmlResponse
   {
      function TemplateModule()
      {
         $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/mutasi_dp3/'.GTFWConfiguration::GetValue('application', 'template_address').'');
         $this->SetTemplateFile('view_mutasi_dp3.html');
      }
      ...
      function ProcessRequest() 
      {
        ...
        if(isset($_GET['id'])){
           ...
           $listDataDp3 = $js->GetListMutasiDp3($id);
        ...
        $return['dataPegawai'] = $dataPegawai;
    		$return['listDataDp3'] = $listDataDp3;
    		return $return;  
      }
      ...
      function ParseTemplate($data = NULL)
      {
        ...
        if ( isset($_GET['dataId'])) {
           $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_dp3', 'updateMutasiDp3', 'do', 'html'));
        }else{
           $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_dp3', 'addMutasiDp3', 'do', 'html'));
        }
      
        $this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('mutasi_dp3', 'Pegawai', 'view', 'html') );
        $this->mrTemplate->AddVar('content', 'URL_BALIK', Dispatcher::Instance()->GetUrl('mutasi_dp3', 'MutasiDp3', 'view', 'html').'&id='.$dataPegawai[0]['id'] );
        $this->mrTemplate->AddVar('content', 'URL_CARI', Dispatcher::Instance()->GetUrl('mutasi_dp3', 'popupPegawai', 'view', 'html'));
       ...
       if (empty($dataDp3)) {
          ...
          $urlDelete = Dispatcher::Instance()->GetUrl('mutasi_jabatan_dp3', 'deleteMutasiDp3', 'do', 'html');
          $urlReturn = Dispatcher::Instance()->GetUrl('mutasi_jabatan_dp3', 'MutasiDp3', 'view', 'html');
          ...
          $total=0;
          $start=1;
          for ($i=0; $i<count($dataDp3); $i++) {
             $no = $i+$start;
             $dataDp3[$i]['number'] = $no;
             if ($no % 2 != 0) {
                $dataDp3[$i]['class_name'] = 'table-common-even';
             }else{
                $dataDp3[$i]['class_name'] = '';
             }
    
            if($i == 0) $this->mrTemplate->AddVar('content', 'FIRST_NUMBER', $no);
            if($i == sizeof($dataDp3)-1) $this->mrTemplate->AddVar('content', 'LAST_NUMBER', $no);
            $dataDp3[$i]['tanggal_penetapan'] = $this->date2string($dataDp3[$i]['tanggal_penetapan']);
            $dataDp3[$i]['mulai'] = $this->date2string($dataDp3[$i]['mulai']);
            $dataDp3[$i]['selesai'] = $this->date2string($dataDp3[$i]['selesai']);
            $dataDp3[$i]['tanggal_penilaian'] = $this->date2string($dataDp3[$i]['tanggal_penilaian']);
            $dataDp3[$i]['tanggal_diterima'] = $this->date2string($dataDp3[$i]['tanggal_diterima']);
          
            $idEnc = Dispatcher::Instance()->Encrypt($dataDp3[$i]['id']);
            $urlAccept = 'mutasi_dp3|deleteMutasiDp3|do|html-id-'.$dataPegawai[0]['id'];
            $urlKembali = 'mutasi_dp3|MutasiDp3|view|html-id-'.$dataPegawai[0]['id'];
            $label = 'Data Mutasi DP3';
            $dataName = $dataDp3[$i]['mulai'].' s/d '.$dataDp3[$i]['selesai'].' dari '.$dataDp3[$i]['yang_dinilai'];
            $dataDp3[$i]['URL_DELETE'] = Dispatcher::Instance()->GetUrl('confirm', 'confirmDelete', 'do', 'html').'&urlDelete='. $urlAccept.'&urlReturn='.$urlKembali.'&id='.$idEnc.'&label='.$label.'&dataName='.$dataName;
            $dataDp3[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_dp3','MutasiDp3', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
            $dataDp3[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_dp3','detailMutasi', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
			$dataDp3[$i]['URL_RTF'] = Dispatcher::Instance()->GetUrl('mutasi_dp3','rtfDP3', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
            if (!empty($dataDp3[$i]['upload'])){
				$dataDp3[$i]['LINK_DOWNLOAD_SK'] = $data['link']['link_download'].$dataDp3[$i]['upload'];
			}else{
				$dataDp3[$i]['LINK_DOWNLOAD_SK'] = '';
				$dataDp3[$i]['VIEW_DOWNLOAD'] = 'none';
			}
            $this->mrTemplate->AddVars('data_item', $dataDp3[$i], 'DP3_');
