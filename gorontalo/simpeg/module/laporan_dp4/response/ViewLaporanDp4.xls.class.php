<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/laporan_dp4/business/laporan.class.php';
   
class ViewLaporanDp4 extends XlsResponse
{
   var $mWorksheets = array('Dp4');
   
   function GetFileName() {
      // name it whatever you want
      return 'Laporan_DP4_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      ...
      if(isset($_POST['awal_year'])) {
		  ...
		  $totalData = $this->Obj->GetCountDataDp4($this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
		  $dataPegawai = $this->Obj->GetDataDp4(0, $totalData, $this->awal,$this->akhir, $this->unit_kerja, $this->pangkat_golongan);
  		$data=$dataPegawai;
  		$row=-1;
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=13;
  		...
  		$row++;
		  $this->mWorksheets['Dp4']->write($row, 0, 'DAFTAR PENILAIAN PELAKSANAAN PEKERJAAN PEGAWAI (DP4)', $this->fH1);
    	$this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
    	
    	$row++;
		  $this->mWorksheets['Dp4']->write($row, 0, 'Periode Awal : '.$this->label_awal , $this->fH2);
		  $this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Dp4']->write($row, 0, 'Periode Akhir : '.$this->label_akhir , $this->fH2);
		  $this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Dp4']->write($row, 0, 'Unit Kerja  : '.$this->label_unit_kerja , $this->fH2);
		  $this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Dp4']->write($row, 0, 'Pangkat/Golongan  : '.$this->label_pangkat_golongan , $this->fH2);
		  $this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  //Set Header
		  $header[0]=array('No.','NIP','Nama','Pangkat','Kesetiaan','Prestasi Kerja','Tanggung Jawab','Keataatan','Kejujuran','Kerja Sama','Prakarsa','Kepemimpinan','Implementasi 7 Nilai Dasar Yayasan','Unit Kerja');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','nip','nama','pangkat','kesetiaan','prestasi_kerja','tanggung_jawab','ketaatan','kejujuran','kerjasama','prakarsa','kepemimpinan','nilai_yayasan','unit_kerja');
      ...
      for ($i=0; $i<sizeof($header); $i++){
        ...
        for ($ii=0; $ii<sizeof($val); $ii++){
          $this->mWorksheets['Dp4']->write($row, $ii, $header[$i][$ii], $this->fH3);
		  ...
		  for ($i=1; $i<=sizeof($merger); $i++){
		    $this->mWorksheets['Dp4']->merge_cells($merger[$i]['row_awal'], $merger[$i]['col_awal'], $merger[$i]['row_akhir'], $merger[$i]['col_akhir']);
      }
      
      $row++;
      for ($i=0; $i<$jumKolom; $i++){
        $this->mWorksheets['Dp4']->write($row, $i, $i+1, $this->fH3);
      }
      
      if (sizeof($data)<=0) {
          $row++;
    			$this->mWorksheets['Dp4']->write($row, 0, 'Data Tidak Ditemukan', $this->fH3);
    			for ($i=1; $i<$jumKolom; $i++){
            $this->mWorksheets['Dp4']->write($row, $i, '', $this->fH3);
          }
    			$this->mWorksheets['Dp4']->merge_cells($row, 0, $row,$jumKolom-1);
    	} else {
    			$dataPegawai = $data;
    			for ($i=0; $i<sizeof($dataPegawai); $i++) {
    					...
    					$dataPegawai[$i]['no'] = $no;
    					for ($ii=0; $ii<sizeof($val); $ii++){
                $this->mWorksheets['Dp4']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData2);
            ...
            $this->mWorksheets['Dp4']->set_column($i,$i,$size_col[$val[$i]]+3);
          }
        }
