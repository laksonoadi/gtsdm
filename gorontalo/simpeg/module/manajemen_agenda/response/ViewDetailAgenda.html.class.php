...
class ViewDetailAgenda extends HtmlResponse {
   ...
   function ParseTemplate($data = NULL) {
      if(!empty($data['agenda'])){
         ...
         $this->mrTemplate->addVar('content','FOTO',GTFWConfiguration::GetValue('application', 'file_agenda').$agenda['FOTO']);
			if($agenda['FOTO'] != '') {
				$this->mrTemplate->addVar('foto', 'FOTO_EMPTY', 'NO');
				$this->mrTemplate->addVar('foto','FOTO',GTFWConfiguration::GetValue('application', 'file_agenda').$agenda['FOTO']);
			} else {
