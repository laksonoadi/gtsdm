...
'module/mutasi_mengajar/business/mutasi_mengajar.class.php';

class ViewMutasiMengajar extends HtmlResponse
   {
      function TemplateModule()
      {
         $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/mutasi_mengajar/'.GTFWConfiguration::GetValue('application', 'template_address').'');
         $this->SetTemplateFile('view_mutasi_mengajar.html');
      }
      
      function ProcessRequest() 
      {
		$mgj = new MutasiMengajar();
		...
		$this->profilId=$id;
         ...
         $dataMgj = $mgj->GetListMutasiMengajar($id);
            if(isset($_GET['dataId'])){
               ...
               $result=$dataMutasi[0];
                  if(!empty($result)){
						...
						$return['input']['kode_mk'] = $result['kode_mk'];
						$return['input']['nama_mk'] = $result['nama_mk'];
						$return['input']['sks'] = $result['sks'];
						$return['input']['kelas'] = $result['kelas'];
				...
				$return['input']['nama_mk'] = '';
				$return['input']['kode_mk'] = '';
				$return['input']['sks'] = '';
				$return['input']['kelas'] = '';
				$return['input']['status'] = '';
				$return['input']['upload'] = '';
		...
		if ($this->StatusIntegrasiAkademik){
			...
			$mgjIntegrasi = new MutasiMengajar($nomorkoneksi);
			$mgjIntegrasi->connect();
			$dataMgj = $mgjIntegrasi->GetListMutasiMengajarIntegrasi($dataPegawai[0]['kode']);
	  ...
	  if($data['lang']=='eng') {
		...
		$this->mrTemplate->AddVar('content', 'TITLE', 'RIWAYAT DOSEN MENGAJAR');
		...
		$this->mrTemplate->AddVar('content', 'TYPE', isset($_GET['dataId']) ? 'submit' : 'reset');
		$label = "Manajemen Data Mutasi Dosen Mengajar";
		$label_data = 'Data Mutasi Dosen Mengajar';
	  }
	  
      if ( isset($_GET['dataId'])) {
         $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'updateMutasiMengajar', 'do', 'html'));
      }else{
         $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'addMutasiMengajar', 'do', 'html'));
      }
      
      $this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'Pegawai', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_BALIK', Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'MutasiMengajar', 'view', 'html').'&id='.$dataPegawai[0]['id'] );
      ...
      $urlDelete = Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'deleteMutasiMengajar', 'do', 'html');
      $urlReturn = Dispatcher::Instance()->GetUrl('mutasi_mengajar', 'MutasiMengajar', 'view', 'html');
      ...
      if($i == sizeof($dataIstri)-1) $this->mrTemplate->AddVar('content', 'LAST_NUMBER', $no);
		 ...
		 $urlAccept = 'mutasi_mengajar|deleteMutasiMengajar|do|html-id-'.$dataPegawai[0]['id'];
         $urlKembali = 'mutasi_mengajar|MutasiMengajar|view|html-id-'.$dataPegawai[0]['id'];
         ...
         $dataMgj[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_mengajar','MutasiMengajar', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
         $dataMgj[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_mengajar','detailMutasi', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
