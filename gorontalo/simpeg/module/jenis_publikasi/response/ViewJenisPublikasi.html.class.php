...
require_once GTFWConfiguration::GetValue( 'application', 'docroot').
   'module/jenis_publikasi/business/jenis_publikasi.class.php';
   
class ViewJenisPublikasi extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/jenis_publikasi/'.GTFWConfiguration::GetValue('application', 'template_address').'');
      $this->SetTemplateFile('view_jenis_publikasi.html');
   }
   
   function ProcessRequest()
   {
      $publik = new JenisPublikasi;
      ...
      $url = Dispatcher::Instance()->GetUrl('jenis_publikasi','jenisPublikasi','view','html').'&cari='.$return['cari'];
      if (isset($_GET['id'])){ 
      $url .= '&id='.$id;
      }
      Messenger::Instance()->SendToComponent('paging', 'Paging', 'view', 'html', 'paging_top', array($itemViewed,$totalData, $url, $currPage), Messenger::CurrentRequest);
      $return['link']['url_action'] = Dispatcher::Instance()->GetUrl('jenis_publikasi','inputJenisPublikasi','do','html');
      if (isset($_GET['id'])){ 
      $return['link']['url_action'] .= '&id='.$id;
      }
      $return['link']['url_search'] = Dispatcher::Instance()->GetUrl('jenis_publikasi','jenisPublikasi','view','html');
      $return['link']['url_edit'] = Dispatcher::Instance()->GetUrl('jenis_publikasi','jenisPublikasi','view','html');
      if ($return['cari'] != ''){
      ...
      $lang=GTFWConfiguration::GetValue('application', 'button_lang');
       if ($lang=='eng'){
          $label="Publication Type Reference";
       }else{
          $label="Referensi Jenis Publikasi";  
       }
       $return['lang']=$lang;
      
      $return['link']['url_delete'] = Dispatcher::Instance()->GetUrl('confirm', 'confirmDelete', 'do', 'html').
      "&urlDelete=".Dispatcher::Instance()->Encrypt('jenis_publikasi|deleteJenisPublikasi|do|html').
      "&urlReturn=".Dispatcher::Instance()->Encrypt('jenis_publikasi|jenisPublikasi|view|html').
      "&label=".Dispatcher::Instance()->Encrypt($label);
      $return['link']['url_delete_js'] = Dispatcher::Instance()->GetUrl('jenis_publikasi', 'deleteJenisPublikasi', 'do', 'html');
   ...
   function ParseTemplate($data = NULL)
   {
     ...
     if ($data['lang']=='eng'){
         $this->mrTemplate->AddVar('content', 'TITLE', 'PUBLICATION TYPE REFERENCE');
         $this->mrTemplate->AddVar('content', 'JUDUL_DATA', 'Publication Type Data');
         $this->mrTemplate->AddVar('content', 'LABEL_ACTION', isset($_GET['id']) ? 'Update' : 'Add');
     }else{
         $this->mrTemplate->AddVar('content', 'TITLE', 'REFERENSI JENIS PUBLIKASI');
         $this->mrTemplate->AddVar('content', 'JUDUL_DATA', 'Data Jenis Publikasi');
