...
class ViewLaporanPensiun extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_pensiun/'.GTFWConfiguration::GetValue('application', 'template_address').'');
      $this->SetTemplateFile('view_laporan_pensiun.html');
   }
   ...
   function ProcessRequest()
   {
      ...
      $totalData = $this->Obj->GetCountDataPensiun($this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
  		...
  		$dataPegawai = $this->Obj->GetDataPensiun($startRec, $itemViewed, $this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_pensiun', 'laporanPensiun', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_pensiun', 'laporanPensiun', 'view', 'xls')
        .'&unit_kerja=' . Dispatcher::Instance()->Encrypt($this->unit_kerja)
        .'&pangkat_golongan=' . Dispatcher::Instance()->Encrypt($this->pangkat_golongan)
        .'&awal=' . Dispatcher::Instance()->Encrypt($this->awal)
        .'&akhir=' . Dispatcher::Instance()->Encrypt($this->akhir)
