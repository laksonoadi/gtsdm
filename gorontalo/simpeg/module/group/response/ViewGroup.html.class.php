...
class ViewGroup extends HtmlResponse {
   ...
   function ProcessRequest() {
      ...
      $dataGroup = $groupObj->GetDataGroupAdminUser('', $applicationId, true);
   ...
   function ParseTemplate($data = NULL) {
      ...
      if (empty($data['dataGroup'])) {
         ...
         for($i=1;$i<count($menuBaru)+1;$i++){            
            ...
            $menuBaruDetail = Dispatcher::Instance()->GetUrl('group', 'DetailGroup', 'view', 'html') . '&groupId=' . $idEnc;
            $this->mrTemplate->AddVar('data_group_item', 'GROUP_URL_DETAIL', $menuBaruDetail);
