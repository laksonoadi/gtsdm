...
class ViewKomponenGaji extends HtmlResponse {
	...
	function TemplateModule() {
		...
		$this->SetTemplateFile('view_komponen_gaji.html');
	}
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('komponen_gaji_karyawan', 'komponenGaji', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('komponen_gaji_karyawan', 'inputKomponen', 'view', 'html'));
		if($this->Pesan) {
...
//mulai bikin tombol delete
      $lang=GTFWConfiguration::GetValue('application', 'button_lang');
	     if ($lang=='eng'){
          $label="Formula Component for Salary";
       }else{
          $label="Komponen Formula Gaji";  
       }
			$urlDelete = Dispatcher::Instance()->GetUrl('komponen_gaji_karyawan', 'deleteKomponen', 'do', 'html');
			$urlReturn = Dispatcher::Instance()->GetUrl('komponen_gaji_karyawan', 'komponenGaji', 'view', 'html');
			...
			for ($i=0; $i<sizeof($dataKomponen); $i++) {
				...
				$dataKomponen[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('komponen_gaji_karyawan', 'inputKomponen', 'view', 'html') . '&dataId=' . $idEnc . '&page=' . $encPage . '&cari='.$cari;
