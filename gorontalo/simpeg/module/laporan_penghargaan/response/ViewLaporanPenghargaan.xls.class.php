<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/laporan_penghargaan/business/laporan.class.php';
   
class ViewLaporanPenghargaan extends XlsResponse
{
   var $mWorksheets = array('Penghargaan');
   
   function GetFileName() {
      // name it whatever you want
      return 'Laporan_Penghargaan_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      ...
      if(isset($_POST['awal_year'])) {
		  ...
		  $totalData = $this->Obj->GetCountDataPenghargaan($this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
		  $dataPegawai = $this->Obj->GetDataPenghargaan(0, $totalData, $this->awal,$this->akhir, $this->unit_kerja, $this->pangkat_golongan);
  		$data=$dataPegawai;
  		$row=-1;
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=11;
  		...
  		$row++;
		  $this->mWorksheets['Penghargaan']->write($row, 0, 'DATA PEGAWAI (YANG AKAN) MENERIMA PENGHARGAAN', $this->fH1);
    	$this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
    	
    	$row++;
		  $this->mWorksheets['Penghargaan']->write($row, 0, 'Periode Awal : '.$this->label_awal , $this->fH2);
		  $this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Penghargaan']->write($row, 0, 'Periode Akhir : '.$this->label_akhir , $this->fH2);
		  $this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Penghargaan']->write($row, 0, 'Unit Kerja  : '.$this->label_unit_kerja , $this->fH2);
		  $this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Penghargaan']->write($row, 0, 'Pangkat/Golongan  : '.$this->label_pangkat_golongan , $this->fH2);
		  $this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  //Set Header
		  $header[0]=array('No','NIP','Nama','Pangkat','Jabatan Struktural','Jabatan Fungsional','Tanggal Masuk Institusi','Masa Kerja','Masa Kerja Penghargaan','Tanggal Penghargaan','Unit Kerja');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','nip','nama','pangkat','jabatan_struktural','jabatan_fungsional','tanggal_masuk_institusi','masa_kerja','masa_kerja_penghargaan','tanggal_penghargaan','unit_kerja');
      ...
      for ($i=0; $i<sizeof($header); $i++){
        ...
        for ($ii=0; $ii<sizeof($val); $ii++){
          $this->mWorksheets['Penghargaan']->write($row, $ii, $header[$i][$ii], $this->fH3);
		  ...
		  for ($i=1; $i<=sizeof($merger); $i++){
		    $this->mWorksheets['Penghargaan']->merge_cells($merger[$i]['row_awal'], $merger[$i]['col_awal'], $merger[$i]['row_akhir'], $merger[$i]['col_akhir']);
      }
      
      $row++;
      for ($i=0; $i<$jumKolom; $i++){
        $this->mWorksheets['Penghargaan']->write($row, $i, $i+1, $this->fH3);
      }
      
      if (sizeof($data)<=0) {
          $row++;
    			$this->mWorksheets['Penghargaan']->write($row, 0, 'Data Tidak Ditemukan', $this->fH3);
    			for ($i=1; $i<$jumKolom; $i++){
            $this->mWorksheets['Penghargaan']->write($row, $i, '', $this->fH3);
          }
    			$this->mWorksheets['Penghargaan']->merge_cells($row, 0, $row,$jumKolom-1);
    	} else {
    			$dataPegawai = $data;
    			for ($i=0; $i<sizeof($dataPegawai); $i++) {
    					...
    					$row++; $cols=0;
    					$dataPegawai[$i]['no'] = $no;
    					$dataPegawai[$i]['tanggal_masuk_institusi']=$this->Obj->IndonesianDate($dataPegawai[$i]['tanggal_masuk_institusi'],'YYYY-MM-DD');
        		  $dataPegawai[$i]['tanggal_penghargaan']=$this->Obj->IndonesianDate($dataPegawai[$i]['tanggal_penghargaan'],'YYYY-MM-DD');
    					for ($ii=0; $ii<sizeof($val); $ii++){
                $this->mWorksheets['Penghargaan']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData2);
            ...
            $this->mWorksheets['Penghargaan']->set_column($i,$i,$size_col[$val[$i]]+3);
          }
        }
