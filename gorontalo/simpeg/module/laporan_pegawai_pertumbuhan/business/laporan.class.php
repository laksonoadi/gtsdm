...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_pegawai_pertumbuhan/business/laporan.sql.php';
  ...
  //==GET== 
	function GetComboUnitKerjaLike($combo=false){
		...
		$result=$this->Open($sql,array($satkerlevel));
		// print_r($sql);
...
//==GET== 
   ...
   function GetTahun($tahun1,$tahun2){
      $tahun1=$this->tahun["'".$tahun1."'"];
      $tahun2=$this->tahun["'".$tahun2."'"];
      if ($tahun1>$tahun2){
        $temp=$tahun1;
        $tahun1=$tahun2;
        $tahun2=$temp;
   ...
   function GetDataPegawai($offset, $limit, $jabatan_fungsional,$jenis_pegawai) {
      ...
      if($jabatan_fungsional != "all"){
  		  $sql=str_replace('%jabatan_fungsional%',' AND jabfungjenisrId='.$jabatan_fungsional ,$sql);
      }else{
        $sql=str_replace('%jabatan_fungsional%','',$sql);
      }
      
      if($jenis_pegawai != "all"){
  		  $sql=str_replace('%jenis_pegawai%',' AND pegJnspegrId='.$jenis_pegawai ,$sql);
      }else{
