<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/laporan_pegawai_pertumbuhan/business/laporan.class.php';
   
class ViewLaporanPegawai extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_pegawai_pertumbuhan/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ProcessRequest()
   {
      ...
      if(isset($_POST['tahun1'])) {
  				$this->tahun1 = $_POST['tahun1'];
  		} elseif(isset($_GET['tahun1'])) {
  				$this->tahun1 = Dispatcher::Instance()->Decrypt($_GET['tahun1']);
  		} else {
  				$this->tahun1 = date('Y')-5;
  		}
  		
  		if(isset($_POST['tahun2'])) {
  				$this->tahun2 = $_POST['tahun2'];
  		} elseif(isset($_GET['tahun2'])) {
  				$this->tahun2 = Dispatcher::Instance()->Decrypt($_GET['tahun2']);
  		} else {
  				$this->tahun2 = date('Y');
  		}
      ...
      $this->ComboTahun=$this->Obj->GetComboTahun();
  		$this->label_tahun1=$this->GetLabelFromCombo($this->ComboTahun,$this->tahun1);
  		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tahun1', 
      array('tahun1', $this->ComboTahun, $this->tahun1, '', ''), Messenger::CurrentRequest);
      
  		$this->label_tahun2=$this->GetLabelFromCombo($this->ComboTahun,$this->tahun2);
  		Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tahun2', 
      array('tahun2', $this->ComboTahun, $this->tahun2, '', ''), Messenger::CurrentRequest);
      ...
      //create paging 
      $this->ComboUnitKerja=$this->Obj->GetComboUnitKerjaLike();
      $this->Tahun=$this->Obj->GetTahun($this->tahun1,$this->tahun2);
      $totalData = sizeof($this->ComboUnitKerja);
  		...
  		for ($i=0; $i<sizeof($dataPegawai); $i++){
  		  $this->dataJumlah[$dataPegawai[$i]['unit_kerja']][$dataPegawai[$i]['tahun_masuk']][$dataPegawai[$i]['jenis_kelamin']]=$dataPegawai[$i]['jumlah'];
      }
        ...
        .'&tahun1=' . Dispatcher::Instance()->Encrypt($this->tahun1)
        .'&tahun2=' . Dispatcher::Instance()->Encrypt($this->tahun2)
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_pegawai_pertumbuhan', 'laporanPegawai', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_pegawai_pertumbuhan', 'laporanPegawai', 'view', 'xls')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&tahun1=' . Dispatcher::Instance()->Encrypt($this->tahun1)
        .'&tahun2=' . Dispatcher::Instance()->Encrypt($this->tahun2)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
      $this->mrTemplate->AddVar('content', 'URL_RTF', Dispatcher::Instance()->GetUrl('laporan_pegawai_pertumbuhan', 'rtfLaporanPegawai', 'view', 'html')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&tahun1=' . Dispatcher::Instance()->Encrypt($this->tahun1)
        .'&tahun2=' . Dispatcher::Instance()->Encrypt($this->tahun2)
        ...
        if (empty($this->ComboUnitKerja)) {
    			...
    			$totalKolomTahun=sizeof($this->Tahun);
    			$this->mrTemplate->AddVar('content', 'GOLONGAN_COLSPAN', $totalKolomTahun*2);
    			
    			//Header Pendidikan
    			for ($i=0; $i<$totalKolomTahun; $i++){
    			   $this->mrTemplate->AddVar('kolom_tahun','GOLONGAN',$this->Tahun[$i]['name']);
    			   $this->mrTemplate->parseTemplate('kolom_tahun', 'a');
          }
          ...
          //Header Jenis Kelamin + 1 dengan kolom jumlah
    			...
    			for ($i=0; $i<sizeof($this->ComboUnitKerja); $i++) {
        			...
        			for ($ii=0; $ii<$totalKolomTahun; $ii++){
        			   $tempDataL=$this->dataJumlah[$this->ComboUnitKerja[$i]['id']][$this->Tahun[$ii]['id']]['L'];
        			   $tempDataP=$this->dataJumlah[$this->ComboUnitKerja[$i]['id']][$this->Tahun[$ii]['id']]['P'];
        			   
        			   if ($tempDataL=='') $tempDataL=0;
                 if ($tempDataP=='') $tempDataP=0;
                 
                 $this->dataJumlah[$this->ComboUnitKerja[$temp]['id']][$this->Tahun[$ii]['id']]['L'] += $tempDataL;
                 $this->dataJumlah[$this->ComboUnitKerja[$temp]['id']][$this->Tahun[$ii]['id']]['P'] += $tempDataP;
