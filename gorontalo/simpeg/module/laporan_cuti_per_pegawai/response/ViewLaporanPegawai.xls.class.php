...
{
   var $mWorksheets = array('Cuti');
   
   function GetFileName() {
      // name it whatever you want
      return 'laporan_cuti_per_pegawai_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      $this->Obj=new Laporan;
  		...
  		if(isset($_POST['nip'])) {
  				...
  				$this->nip = Dispatcher::Instance()->Decrypt($_GET['nip']);
  		} else {
  				$this->nip = 'all';
  		}
		 ...
		 //print_r($totalData);die;
		  $dataPegawai = $this->Obj->GetDataCutiXls($this->nip);
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=7;
  		$row++;
		  $this->mWorksheets['Cuti']->write($row, 0, 'DATA CUTI PER PEGAWAI', $this->fH1);
    	$this->mWorksheets['Cuti']->merge_cells($row, 0, $row,$jumKolom-1);
    			  
		  $row++;
		  $this->mWorksheets['Cuti']->write($row, 0, 'NIP-Nama Pegawai  : '.$this->label_nip , $this->fH2);
		  $this->mWorksheets['Cuti']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  //Set Header
		  $header[0]=array('No.','No Cuti','Tipe Cuti','Tanggal Mulai','Tanggal Selesai','Status','Tanggung Jawab Kerja');
		  //$header[1]=array('','','','','','Golongan','Jabatan','','','');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','no_cuti','nama','mulai','selesai','status','tggjwb');
      ...
      for ($i=0; $i<sizeof($header); $i++){
        ...
        for ($ii=0; $ii<sizeof($val); $ii++){
          $this->mWorksheets['Cuti']->write($row, $ii, $header[$i][$ii], $this->fH3);
		  ...
		  for ($i=1; $i<=sizeof($merger); $i++){
		    $this->mWorksheets['Cuti']->merge_cells($merger[$i]['row_awal'], $merger[$i]['col_awal'], $merger[$i]['row_akhir'], $merger[$i]['col_akhir']);
      }
      //Menulis Angka sebelum data
      $row++;
      for ($i=0; $i<$jumKolom; $i++){
        $this->mWorksheets['Cuti']->write($row, $i, $i+1, $this->fH3);
      }
      
      if (sizeof($data)<=0) {
          $row++;
    			$this->mWorksheets['Cuti']->write($row, 0, 'Data Tidak Ditemukan', $this->fH3);
    			for ($i=1; $i<$jumKolom; $i++){
            $this->mWorksheets['Cuti']->write($row, $i, '', $this->fH3);
          }
    			$this->mWorksheets['Cuti']->merge_cells($row, 0, $row,$jumKolom-1);
    	} else {
    			$dataPegawai = $data;
              ...
              //Menulis Datanya
              for ($ii=0; $ii<sizeof($val); $ii++){
                $this->mWorksheets['Cuti']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData2);
            ...
            $this->mWorksheets['Cuti']->set_column($i,$i,$size_col[$val[$i]]+3);
          }
