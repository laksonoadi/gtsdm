...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_cuti_seluruh_pegawai/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetCountDisetujui($idPeg, $idper){
	$result= $this->Open($this->mSqlQueries['get_count_disetujui'],array($idPeg, $idper));
		return $result[0]['disetujui'];
	  //print_r($this->getLastError());exit;
   }
   function GetCountDitolak($idPeg, $idper){
   $result= $this->Open($this->mSqlQueries['get_count_ditolak'],array($idPeg, $idper));
		return $result[0]['ditolak'];
	  // print_r($this->getLastError());exit;
   }
   function GetCountProses($idPeg, $idper){
   $result= $this->Open($this->mSqlQueries['get_count_proses'],array($idPeg, $idper));
		return $result[0]['diproses'];
	  //print_r($this->getLastError());exit;
   }
   
   function GetCountDataCutiPegawai($unit_kerja) {
      $sql=$this->mSqlQueries['get_data_cuti_pegawai']; 
      
      if($unit_kerja != "all"){
  		  $sql=str_replace('%unit_kerja%',' AND satkerpegSatkerId='.$unit_kerja ,$sql);
      }else{
   ...
   function GetDataCutiSeluruhPegawai($offset, $limit, $unit_kerja) {
      $sql=$this->mSqlQueries['get_data_cuti_pegawai']; 
