...
class ViewUpdatePolicy extends HtmlResponse {
   ...
   function ProcessRequest() {
      ...
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'tipe', array('tipe',$comboTipe,$data['policy'][0]['jnspolicyid'],'false',''), Messenger::CurrentRequest);
   ...
   function CheckRequiredFields() {
      ...
      //$this->CheckRequired($_POST['tanggal_policy'], 'Tanggal harus diisi');
		...
		// Check satker
		if(trim($_POST['satkerpolicy']->Raw()) === '') {
			$this->validation = FALSE;
			$this->mMessage[] = "Unit Kerja tidak boleh kosong";
		}
		// Check tipe
		if(trim($_POST['tipe']->Raw()) === '') {
			$this->validation = FALSE;
			$this->mMessage[] = "Jenis tidak boleh kosong";
		}
		// Check nama
		if(trim($_POST['nama']->Raw()) === '') {
			$this->validation = FALSE;
			$this->mMessage[] = "Nama tidak boleh kosong";
		}
   }
