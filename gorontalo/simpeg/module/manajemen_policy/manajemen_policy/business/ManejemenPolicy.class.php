...
class ManejemenPolicy extends Database {
   ...
   function ListPolicy($nama_deskripsi='', $satuan_kerja='', $jenis_policy='',$offset, $limit) {
      if($nama_deksripsi != ""){
  		  $str = " WHERE (policyNama LIKE '%".$nama_deskripsi."%' OR policyKeterangan LIKE '%".$nama_deskripsi."%')";
      }else{
        $str = "";
      }
      
      if($satuan_kerja != "all"){
  		  $str .= " WHERE policySatkerpolicyId=".$satuan_kerja;
      }else{
        $str .= "";
      }
      
      if($jenis_policy != "all"){
  		  $str .= " WHERE policyJnspolicyId=".$jenis_policy;
      }else{
        $str .= "";
      }
   ...
   function getIdMax(){
      $buff = $this->Open($this->mSqlQueries['get_id_max'], array());
      return $buff[0]['IDMAX'];
   }
   ...
   function GetPegawaiPolicyById($id){
      return $this->Open($this->mSqlQueries['get_pegawai_policy_by_id'], array($id));
   }
   ...
   function AddDetailPolicy($array){
   //echo vsprintf($this->mSqlQueries['add_policy'],$array);
      return $this->Execute($this->mSqlQueries['add_detail_policy'],$array);
   }
   ...
   function DeletePegPolicy($id){
	//echo sprintf($this->mSqlQueries['delete_policy'],$id);exit;
      return $this->Execute($this->mSqlQueries['delete_pegawai_policy'],array($id));
   }
   
   function DeletePegIdPolicy($id){
	//echo sprintf($this->mSqlQueries['delete_policy'],$id);exit;
      return $this->Execute($this->mSqlQueries['delete_pegawai_id_policy'],array($id));
   }
   ...
   function IndonesianDate($StrDate, $StrFormat)
	{
		...
		switch ($Month)
		{
			case "01" :	$StrResult = $Day." Januari ".$Year;
						break;
			case "02" :	$StrResult = $Day." Februari ".$Year;
						break;
