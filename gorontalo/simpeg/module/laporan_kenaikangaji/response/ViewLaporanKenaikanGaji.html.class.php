...
class ViewLaporanKenaikanGaji extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_kenaikangaji/'.GTFWConfiguration::GetValue('application', 'template_address').'');
      $this->SetTemplateFile('view_laporan_kenaikangaji.html');
   }
   ...
   function ProcessRequest()
   {
      ...
      $totalData = $this->Obj->GetCountDataKenaikanGaji($this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
  		...
  		$dataPegawai = $this->Obj->GetDataKenaikanGaji($startRec, $itemViewed, $this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_kenaikangaji', 'laporanKenaikanGaji', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_kenaikangaji', 'laporanKenaikanGaji', 'view', 'xls')
        .'&unit_kerja=' . Dispatcher::Instance()->Encrypt($this->unit_kerja)
        .'&pangkat_golongan=' . Dispatcher::Instance()->Encrypt($this->pangkat_golongan)
        .'&awal=' . Dispatcher::Instance()->Encrypt($this->awal)
        .'&akhir=' . Dispatcher::Instance()->Encrypt($this->akhir)
        ...
        if (empty($data['dataPegawai'])) {
    			...
    			$total=0;
				$urlCetak = Dispatcher::Instance()->GetUrl('laporan_kenaikangaji', 'skLaporanKenaikanGaji', 'view', 'html');
				...
				$encId = Dispatcher::Instance()->Encrypt($dataPegawai[$i]['nip']);
