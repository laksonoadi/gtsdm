...
$sql['get_list_mutasi_organisasi_pegawai_verifikasi']="
SELECT DISTINCT 
	p.orgId AS id,
	p.orgPegKode AS nip,
	jnsorgNama AS jenis_label,
...
FROM
	sdm_organisasi p
	LEFT JOIN sdm_ref_jenis_organisasi ON p.orgJenis=jnsorgId
	INNER JOIN sdm_verifikasi_data vd ON vd.`verdataValue`=p.`orgId` AND vd.`verdataStatus`='3' AND vd.`verdataVerifikasiId`='4'
WHERE 
   p.orgPegKode='%s'
"; 
