...
FROM
	(SELECT
		...
		mkPenyesuaianTahun AS tahunPenyesuaian,
		mkPenyesuaianBulan AS bulanPenyesuaian,
		pendId,
	...
	FROM
		...
		LEFT JOIN sdm_status_pegawai k ON k.statpegId=pegId AND k.statpegAktif='Aktif'
		LEFT JOIN sdm_ref_status_pegawai ON statrId=k.statpegStatrId
		LEFT JOIN pub_ref_agama ON agmId=pegAgamaId
		LEFT JOIN sdm_istri_suami ON sutriPegId=pegId
		LEFT JOIN (select SUM(mkPenyesuaianTahun) as mkPenyesuaianTahun,
						SUM(mkPenyesuaianBulan) as mkPenyesuaianBulan,
						mkPegKode as mkPegKode
						FROM sdm_masa_kerja_penyesuaian
			) as masa ON pegId=masa.mkPegKode
	WHERE
...
WHERE 1=1 AND NOT (%notnull% IS NULL)
GROUP BY pegId
ORDER BY %order%
%LIMIT%
";
...
$sql['get_count_daftar_pegawai']="
	...
	FROM
		...
		LEFT JOIN pub_ref_agama ON agmId=pegAgamaId
		LEFT JOIN `sdm_masa_kerja_penyesuaian` ON pegId=mkPegKode
		LEFT JOIN sdm_status_pegawai k ON k.statpegId=pegId AND k.statpegAktif='Aktif'
		LEFT JOIN sdm_verifikasi_data ON pegId=verdataValue 
	WHERE
		1=1 AND verdataStatus='3' AND verdataVerifikasiId='19'
