...
class ViewDetailSertifikasi extends HtmlResponse{
	...
	function ParseTemplate($data = NULL){
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('mutasi_sertifikasi', 'detailSertifikasi', 'view', 'html')); 
		$this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('mutasi_sertifikasi', 'detailSertifikasi', 'view', 'xls').'&srtfkId='.$this->data['srtfkId'].'&srtfkdetHasilAkhir='.$this->data['srtfkdetHasilAkhir']);
		...
		if(empty($cariData)) {
			...
			for ($i=0; $i<sizeof($cariData); $i++) {
				...
				$cariData[$i]['url_detail']=Dispatcher::Instance()->GetUrl('mutasi_sertifikasi', 'detailPesertaSertifikasi', 'view', 'html').'&srtfkdetTahun='.$cariData[$i]['srtfkdetTahun'].'&srtfkdetPegId='.$cariData[$i]['srtfkdetPegId'].'&srtfkId='.$cariData[$i]['srtfkdetSrtfkId'];
