...
class ViewDataPegawai extends HtmlResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $totalData = $Obj->GetCountData($nip_nama, $status_kerja);
  		$itemViewed = 50;
  		$currPage = 1;
  		$startRec = 0 ;
  		if(isset($_GET['page'])) {
  		...
  		$dataPegawai = $Obj->GetData($startRec, $itemViewed, $nip_nama, $status_kerja);
...
//      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'status_kerja', array('status_kerja', $arrStatusKerja, $status_kerja, 'true', ' style="width:130px;" '), Messenger::CurrentRequest);
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'inputDataPegawai', 'view', 'html').'&op=add');
      $this->mrTemplate->AddVar('content', 'URL_CONTRACT', Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'kontrakPegawai', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'dataPegawai', 'view', 'html') );
  ...
  //mulai bikin tombol delete
  			$urlDelete = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'deleteDataPegawai', 'do', 'html');
  			$urlReturn = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'dataPegawai', 'view', 'html');
        ...
        $total=0;
          ...
          $urlAccept = 'data_pegawai_calon|deleteDataPegawai|do|html';
          $urlKembali = 'data_pegawai_calon|dataPegawai|view|html';
          ...
          $dataPegawai[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('data_pegawai_calon','inputDataPegawai', 'view', 'html').'&dataId='. $idEnc;
          $dataPegawai[$i]['URL_DITERIMA'] = Dispatcher::Instance()->GetUrl('data_pegawai','inputDataPegawai', 'view', 'html').'&dataId='. $idEnc;
          $dataPegawai[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('data_pegawai_calon','detailDataPegawai', 'view', 'html').'&dataId='. $idEnc;
