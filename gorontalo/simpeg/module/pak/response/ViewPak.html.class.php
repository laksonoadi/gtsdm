...
class ViewPak extends HtmlResponse {
  ...
  function ProcessRequest() {
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('pak', 'Pak', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('pak', 'inputPak', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'TITLE', 'PENETAPAN ANGKA KREDIT');
      ...
      if (empty($data['pak'])) {
         ...
         $label = "Manajemen Referensi";
			$urlDelete = Dispatcher::Instance()->GetUrl('pak', 'deletePak', 'do', 'html');
			$urlReturn = Dispatcher::Instance()->GetUrl('pak', 'Pak', 'view', 'html');
         ...
         foreach ($data['pak'] as $key => $value) {
            ...
            $urlAccept = 'pak|deletePak|do|html';
            $urlKembali = 'pak|Pak|view|html';
            $label = 'Unsur Penetapan Angka Kredit';
            $dataName = $value['nama'];
            ...
            $value['URL_UPDATE'] = Dispatcher::Instance()->GetUrl('pak','inputPak', 'view', 'html').'&id='.Dispatcher::Instance()->Encrypt($value['id']);
