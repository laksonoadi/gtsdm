...
class ViewPenilaianSertifikasi extends HtmlResponse{
	function TemplateModule(){
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/sertifikasi_penilaian/'.GTFWConfiguration::GetValue('application', 'template_address').'');
		$this->SetTemplateFile('view_penilaian_sertifikasi.html');
	}
	...
	function ProcessRequest(){
		...
		if ($_GET['srtfkId']==''){
			...
			$this->srtfkPeserta = $Obj->GetListPesertaSertifikasiByIdVerify($_GET['srtfkId'],$this->keyword);	
		}
	...
	function ParseTemplate($data = NULL){
		...
		$this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('sertifikasi_penilaian', 'historyDataSertifikasi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('sertifikasi_penilaian', 'penilaianSertifikasi', 'view', 'html').'&srtfkId='.$_GET['srtfkId']);
		$this->mrTemplate->AddVar('content', 'URL_AKSI', Dispatcher::Instance()->GetUrl('sertifikasi_penilaian', 'inputDataSertifikasi', 'do', 'html')); 
		$this->mrTemplate->AddVar('content', 'URL_POPUP_PEGAWAI', Dispatcher::Instance()->GetUrl('sertifikasi_penilaian', 'popupPegawai', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'xls').'&source=penilaian&srtfkId='.($_GET['srtfkId']==''?'ALL':$_GET['srtfkId']).'&srtfkdetHasilAkhir=LULUS');
		$this->mrTemplate->AddVar('content', 'URL_EXCEL_ALL', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'xls').'&source=penilaian&srtfkId=ALL&srtfkdetHasilAkhir=LULUS');
		$this->mrTemplate->AddVar('content', 'URL_EXCEL_NO', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'xls').'&source=penilaian&srtfkId='.($_GET['srtfkId']==''?'ALL':$_GET['srtfkId']).'&srtfkdetHasilAkhir=BELUM LULUS');
		$this->mrTemplate->AddVar('content', 'URL_EXCEL_ALL_NO', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'xls').'&source=penilaian&srtfkId=ALL&srtfkdetHasilAkhir=BELUM LULUS');
