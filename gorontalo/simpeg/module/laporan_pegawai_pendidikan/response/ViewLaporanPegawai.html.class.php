<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/laporan_pegawai_pendidikan/business/laporan.class.php';
   
class ViewLaporanPegawai extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_pegawai_pendidikan/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ProcessRequest()
   {
      ...
      $totalData = sizeof($this->ComboUnitKerja);
  		...
  		for ($i=0; $i<sizeof($dataPegawai); $i++){
  		  $this->dataJumlah[$dataPegawai[$i]['unit_kerja']][$dataPegawai[$i]['pendidikan']][$dataPegawai[$i]['jenis_kelamin']]=$dataPegawai[$i]['jumlah'];
      }
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_pegawai_pendidikan', 'laporanPegawai', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_pegawai_pendidikan', 'laporanPegawai', 'view', 'xls')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
      $this->mrTemplate->AddVar('content', 'URL_RTF', Dispatcher::Instance()->GetUrl('laporan_pegawai_pendidikan', 'rtfLaporanPegawai', 'view', 'html')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
        ...
        if (empty($this->ComboUnitKerja)) {
    			...
    			$totalKolomPendidikan=sizeof($this->ComboPendidikan);
    			$this->mrTemplate->AddVar('content', 'GOLONGAN_COLSPAN', $totalKolomPendidikan*2);
    			
    			//Header Pendidikan
    			for ($i=0; $i<$totalKolomPendidikan; $i++){
    			   $this->mrTemplate->AddVar('kolom_pendidikan','GOLONGAN',$this->ComboPendidikan[$i]['name']);
    			   $this->mrTemplate->parseTemplate('kolom_pendidikan', 'a');
          }
          ...
          //Header Jenis Kelamin + 1 dengan kolom jumlah
    			...
    			for ($i=0; $i<sizeof($this->ComboUnitKerja); $i++) {
        			...
        			for ($ii=0; $ii<$totalKolomPendidikan; $ii++){
        			   $tempDataL=$this->dataJumlah[$this->ComboUnitKerja[$i]['id']][$this->ComboPendidikan[$ii]['id']]['L'];
        			   $tempDataP=$this->dataJumlah[$this->ComboUnitKerja[$i]['id']][$this->ComboPendidikan[$ii]['id']]['P'];
        			   
        			   if ($tempDataL=='') $tempDataL=0;
                 if ($tempDataP=='') $tempDataP=0;
                 
                 $this->dataJumlah[$this->ComboUnitKerja[$temp]['id']][$this->ComboPendidikan[$ii]['id']]['L'] += $tempDataL;
                 $this->dataJumlah[$this->ComboUnitKerja[$temp]['id']][$this->ComboPendidikan[$ii]['id']]['P'] += $tempDataP;
