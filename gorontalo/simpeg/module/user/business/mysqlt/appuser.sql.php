...
$sql['get_count_data_user'] = "
   ...
   WHERE
      unitappApplicationId = '%s' AND UserName<>'admingt' AND
      UserName like '%s' AND
      RealName like '%s'
      GROUP BY u.UserId
";
...
$sql['get_satuan_kerja_by_id']="
SELECT * FROM `pub_satuan_kerja` WHERE `satkerNama` = '%s'
";
//OK
$sql['get_data_user'] = "
   SELECT 
      ...
      CONCAT('[',UnitName,']<br>',satkerNama) AS unit_name/*,
   ...
   FROM
      ...
      LEFT JOIN gtfw_unit_application ua ON ua.unitappId = g.UnitappId
      LEFT JOIN gtfw_unit ON unitappUnitId = UnitId
	  LEFT JOIN gtfw_user_satuan_kerja ON userunitUserId=u.userId
	  LEFT JOIN pub_satuan_kerja ON satkerId=userunitSatuanKerjaId
   WHERE
      unitappApplicationId = '%s' AND UserName<>'admingt' AND
      UserName like '%s' %s
      RealName like '%s'
...
$sql['get_data_user_by_id'] = "
   SELECT 
      ...
      UnitId AS unit_kerja_id,
	   pb.`satkerNama` AS satker_nama,
      userunitSatuanKerjaId AS satuan_kerja_id
   FROM
      gtfw_user u
      LEFT JOIN gtfw_user_def_group dg ON u.UserId = dg.UserId
      LEFT JOIN gtfw_group g ON dg.GroupId = g.GroupId
      ...
      LEFT JOIN gtfw_unit ON unitappUnitId = UnitId
      LEFT JOIN gtfw_user_satuan_kerja ON userunitUserId=u.userId
      LEFT JOIN `pub_satuan_kerja` pb ON pb.`satkerId`= userunitSatuanKerjaId
   WHERE
      u.UserId = '%s'
";
//OK
...
$sql['get_combo_satuan_kerja'] = "
   ...
   WHERE satkerUnitId LIKE '%s'
   ORDER BY satkerParentId
";
...
$sql['get_data_group_by_unit_id'] = "
   ...
   WHERE
      GroupName LIKE '%s' AND unitappUnitId = '%s' AND unitappApplicationId = '%s' AND GroupId != '59'
   ORDER BY 
      GroupName
";
...
SELECT
	satkerId AS id,
	satkerNama AS name,
	satkerParentId as parent_id
FROM pub_satuan_kerja
";

$sql['get_satker_by_user_id'] = "
SELECT
	satkerId AS id,
	satkerNama AS name,
	satkerParentId as parent_id
FROM gtfw_user_satuan_kerja
INNER JOIN pub_satuan_kerja ON userunitSatuanKerjaId=satkerId
...
";
//OK
$sql['do_add_user_satuan_kerja'] = "
   INSERT INTO gtfw_user_satuan_kerja
   SET
      userunitUserId = %s,
      userunitSatuanKerjaId = %s
";

$sql['do_add_user_satuan_kerja_grup'] = "
   INSERT INTO gtfw_unit_group
   SET
      uGroupUnidId = %s,
      uGroupList = %s
";
...
";
//OK
$sql['do_update_user_satuan_kerja'] = "
   UPDATE gtfw_user_satuan_kerja
   SET
      userunitSatuanKerjaId = %s
   WHERE
      userunitUserId = %s
";
$sql['do_update_user_satuan_kerja_grup'] = "
   UPDATE gtfw_unit_group
   SET
      uGroupList = %s
   WHERE
      uGroupUnidId = %s
...
$sql['do_delete_user_by_array_id'] = 
   ...
   WHERE 
      UserId IN ('%s')";
//OK
$sql['do_delete_unit_user'] = 
   "DELETE from gtfw_user_satuan_kerja
   WHERE 
      userunitUserId IN ('%s')";
