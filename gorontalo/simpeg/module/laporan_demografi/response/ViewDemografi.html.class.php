...
class ViewDemografi extends HtmlResponse{

  function TemplateModule(){
    $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
    'module/laporan_demografi/'.GTFWConfiguration::GetValue('application', 'template_address').'');
    $this->SetTemplateFile('view_demografi.html');
  }
  
  function ProcessRequest(){
    $this->Obj = new Demografi;
    $data['link']['url_search'] = Dispatcher::Instance()->GetUrl('laporan_demografi','demografi','view','html');
    $data['link']['url_cetak'] = Dispatcher::Instance()->GetUrl('laporan_demografi','cetakDemografi','view','html').'&cari=1';
    $data['link']['url_excel'] = Dispatcher::Instance()->GetUrl('laporan_demografi','excelDemografi','view','xls').'&cari=1';
    $data['link']['url_grafik'] = Dispatcher::Instance()->GetUrl('laporan_demografi','grafikDemografi','view','img').'&cari=1';
    ...
    if (isset($_POST['btncari'])){
        ...
        if ($_POST[$val]=='on'){
          ...
          array_push($data['kategori']['judul'],'JABATAN FUNGSIONAL');
        }
        //Lama Kerja
    ...
    for ($j=0; $j<sizeof($data['kategori']['id']); $j++){
        ...
        $data['link']['url_cetak'] .='&'.$val.'=on';
        $data['link']['url_excel'] .='&'.$val.'=on';
        $data['link']['url_grafik'] .='&'.$val.'=on';
        
        $data[$val]['checked']='checked';
        unset($list);
        if ($val=='manpower'){
          $list[0]['id']=0; $list[0]['label']='Total Pegawai';
          ...
          $list[4]['id']=55; $list[4]['id2']=200; $list[4]['label']='55 Ke atas';
        }else{
  ...
  function ParseTemplate($data = NULL){
    ...
    $this->mrTemplate->AddVar('content', 'URL_GRAFIK', $data['link']['url_grafik']);
