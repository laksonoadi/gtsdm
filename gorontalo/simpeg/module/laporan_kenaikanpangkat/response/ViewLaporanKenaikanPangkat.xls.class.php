...
class ViewLaporanKenaikanPangkat extends XlsResponse
{
   var $mWorksheets = array('KenaikanPangkat');
   
   function GetFileName() {
      // name it whatever you want
      return 'Laporan_Kenaikan_Pangkat_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      ...
      if(isset($_POST['awal_year'])) {
		  ...
		  $dataPegawai = $this->Obj->GetDataKenaikanPangkat(0, $totalData, $this->awal,$this->akhir, $this->unit_kerja, $this->pangkat_golongan);
  		$data=$dataPegawai;
  		$row=-1;
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=8;
  		...
  		$row++;
		  $this->mWorksheets['KenaikanPangkat']->write($row, 0, 'DAFTAR KENAIKAN PANGKAT', $this->fH1);
    	$this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
    	
    	$row++;
		  $this->mWorksheets['KenaikanPangkat']->write($row, 0, 'Periode Awal : '.$this->label_awal , $this->fH2);
		  $this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['KenaikanPangkat']->write($row, 0, 'Periode Akhir : '.$this->label_akhir , $this->fH2);
		  $this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['KenaikanPangkat']->write($row, 0, 'Unit Kerja  : '.$this->label_unit_kerja , $this->fH2);
		  $this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['KenaikanPangkat']->write($row, 0, 'Pangkat/Golongan  : '.$this->label_pangkat_golongan , $this->fH2);
		  $this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  //Set Header
		  $header[0]=array('No','NIP','Nama','Pangkat','TMT','Pangkat Berikut','','Unit Kerja');
		  $header[1]=array('','','','','','Pangkat','TMT','');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','nip','nama','golongan','golongan_tmt','golongan_yad','golongan_yad_tmt','unit_kerja');
      ...
      for ($i=0; $i<sizeof($header); $i++){
        ...
        for ($ii=0; $ii<sizeof($val); $ii++){
          $this->mWorksheets['KenaikanPangkat']->write($row, $ii, $header[$i][$ii], $this->fH3);
		  ...
		  for ($i=1; $i<=sizeof($merger); $i++){
		    $this->mWorksheets['KenaikanPangkat']->merge_cells($merger[$i]['row_awal'], $merger[$i]['col_awal'], $merger[$i]['row_akhir'], $merger[$i]['col_akhir']);
      }
      
      $row++;
      for ($i=0; $i<$jumKolom; $i++){
        $this->mWorksheets['KenaikanPangkat']->write($row, $i, $i+1, $this->fH3);
      }
      
      if (sizeof($data)<=0) {
          $row++;
    			$this->mWorksheets['KenaikanPangkat']->write($row, 0, 'Data Tidak Ditemukan', $this->fH3);
    			for ($i=1; $i<$jumKolom; $i++){
            $this->mWorksheets['KenaikanPangkat']->write($row, $i, '', $this->fH3);
          }
    			$this->mWorksheets['KenaikanPangkat']->merge_cells($row, 0, $row,$jumKolom-1);
    	} else {
    			$dataPegawai = $data;
    			for ($i=0; $i<sizeof($dataPegawai); $i++) {
        		  ...
        		  $dataPegawai[$i]['golongan_yad_tmt']=$this->Obj->IndonesianDate($dataPegawai[$i]['golongan_yad_tmt'],'YYYY-MM-DD');
    					for ($ii=0; $ii<sizeof($val); $ii++){
                $this->mWorksheets['KenaikanPangkat']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData2);
            ...
            $this->mWorksheets['KenaikanPangkat']->set_column($i,$i,$size_col[$val[$i]]+3);
          }
        }
