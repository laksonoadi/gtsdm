...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_pak_baru/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetCountDataPak($awal, $akhir, $unit_kerja) {
      $sql=$this->mSqlQueries['get_data_pak']; 
   ...
   function GetDataPak($offset, $limit, $awal, $akhir, $unit_kerja) {
      $sql=$this->mSqlQueries['get_data_pak']; 
      $list=$this->Obj->GetListIdAnakUnitByUnitId($unit_kerja);
      if($unit_kerja != "all"){
  		  $sql=str_replace('%unit_kerja%',' AND satkerpegSatkerId IN ('.$list.')' ,$sql);
