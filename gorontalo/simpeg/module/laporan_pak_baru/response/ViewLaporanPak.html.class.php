...
class ViewLaporanPak extends HtmlResponse
{
   ...
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_pak_baru/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ProcessRequest()
   {
      ...
      $totalData = $this->Obj->GetCountDataPak($this->awal,$this->akhir,$this->unit_kerja);
        ...
        //$return['dataTotalAwal'] = $dataTotalAwal;
  		return $return;
   }
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_pak_baru', 'laporanPak', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_pak_baru', 'laporanPak', 'view', 'xls')
        .'&unit_kerja=' . Dispatcher::Instance()->Encrypt($this->unit_kerja)
        .'&awal=' . Dispatcher::Instance()->Encrypt($this->awal)
        .'&akhir=' . Dispatcher::Instance()->Encrypt($this->akhir)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
      $this->mrTemplate->AddVar('content', 'URL_RTF', Dispatcher::Instance()->GetUrl('laporan_pak_baru', 'rtfLaporanPak', 'view', 'html')
        .'&unit_kerja=' . Dispatcher::Instance()->Encrypt($this->unit_kerja)
        .'&awal=' . Dispatcher::Instance()->Encrypt($this->awal)
        .'&akhir=' . Dispatcher::Instance()->Encrypt($this->akhir)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
        ...
        if (empty($data['dataPegawai'])) {
				...
				//$baru = $dataPegawai[$i]['total'];
				//$total = $awal_lama + $baru;
				//$lama[$jenis]+=$data['dataTotal'][$i]['total'];
				//}
				
				//$totallama = $lama['Utama'] + $lama['Penunjang'];
				//$this->mrTemplate->AddVars('content', $totallama,'TOTAL');
    			$total= 0;
				...
				//print_r($totalawal);exit;
				//print_r($dataPegawai[$i]['no']);exit;
				//$this->mrTemplate->AddVars('content', 'TOTALLAMA', '');
      }      
   }
}
