...
'module/mutasi_seminar/business/mutasi_seminar.class.php';
   ...
   class ViewDetailMutasi extends HtmlResponse
   {
      ...
      function ProcessRequest()
      {
         ...
         $return['link']['url_back'] = Dispatcher::Instance()->GetUrl('mutasi_seminar','MutasiSeminar','view','html').'&id='.$profilId;
      ...
      function ParseTemplate($data = NULL)
      {
         ...
         if($data['lang'] = 'eng'){
           $this->mrTemplate->AddVar('content', 'TITLE', 'SEMINARS MUTATION DETAIL');
        } else {
          $this->mrTemplate->AddVar('content', 'TITLE', 'DETAIL RIWAYAT SEMINAR');
        }
         ...
         if(!empty($data['profil'])){
            if (!file_exists(GTFWConfiguration::GetValue( 'application', 'photo_save_path').$data['profil']['foto']) | empty($data['profil']['foto'])) { 
              ...
              $data['profil']['foto'] = $data['profil']['foto'];
            }
