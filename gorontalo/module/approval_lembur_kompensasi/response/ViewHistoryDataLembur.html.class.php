...
class ViewHistoryDataLembur extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/approval_lembur_kompensasi/'.GTFWConfiguration::GetValue('application', 'template_address').'');
	...
	function ParseTemplate($data = NULL) {	  	  
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('approval_lembur_kompensasi', 'historyDataLembur', 'view', 'html') . "&dataId=" . $this->encDataId);
		if($this->Pesan) {
    ...
    //tampilkan history lembur
        ...
        $dataHistory[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('approval_lembur_kompensasi', 'deleteDataLembur', 'do', 'html') . '&dataId=' . $idEnc;
        $dataHistory[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('approval_lembur_kompensasi', 'dataLembur', 'view', 'html') . '&op=edit' . '&dataId=' . $data['idPegawai'] . '&pilihpegawai='.$data['pilihpegawai'].'&dataId2=' . $idEnc;
        $dataHistory[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('approval_lembur_kompensasi', 'detailDataLembur', 'view', 'html') . '&dataId=' . $data['idPegawai'] . '&pilihpegawai='.$data['pilihpegawai'].'&dataId2=' . $idEnc;
