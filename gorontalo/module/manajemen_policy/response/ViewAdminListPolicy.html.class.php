...
class ViewAdminListPolicy extends HtmlResponse {
   ...
   function ProcessRequest() {
      ...
      $url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, Dispatcher::Instance()->mSubModule, Dispatcher::Instance()->mAction, Dispatcher::Instance()->mType .
	    ...
	    $arrSatuanKerja = $objectPolicy->GetComboSatuanKerja();
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'satuan_kerja', array('satuan_kerja', $arrSatuanKerja, $satuan_kerja, 'true', ' style="width:250px;" '), Messenger::CurrentRequest);

	    $arrJenisPolicy = $objectPolicy->GetComboJenisPolicy();
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_policy', array('jenis_policy', $arrJenisPolicy, $jenis_policy, 'true', ' style="width:80px;" '), Messenger::CurrentRequest);
   ...
   function ParseTemplate($data = NULL) {
      ...
      if (($_GET['op']=='edit')&&($_GET['sukses']==1)){
          $message='perubahan data berhasil dilakuakan';
          $css='notebox-done';
      }else if (($_GET['op']=='edit')&&($_GET['sukses']==0)){
          $message='perubahan data gagal dilakuakan';
          $css='notebox-warning';
      }else if (($_GET['op']=='add')&&($_GET['sukses']==1)){
          $message='penambahan data berhasil dilakuakan';
          $css='notebox-done';
      }else if (($_GET['op']=='add')&&($_GET['sukses']==0)){
          $message='penambahan data gagal dilakuakan';
          $css='notebox-warning';
      }else if (($_GET['op']=='delete')&&($_GET['sukses']==1)){
		  $message='Maap Data Tidak Dapat Dihapus, Hapus File Terlebih Dahulu';
          $css='notebox-warning';
	  }else if (($_GET['op']=='delete')&&($_GET['sukses']==1)){
          $message='penghapusan data berhasil dilakuakan';
          $css='notebox-done';
      }else if (($_GET['op']=='delete')&&($_GET['sukses']==0)){
          $message='penghapusan data gagal dilakuakan';
          $css='notebox-warning';
      }
