...
SELECT
  ...
  dp4refId as idref,
	dp4refNama as nama,
	ifnull(dp4nAngkaLama,0) as lama,
	ifnull(dp4nAngkaBaru,0) as baru,
	ifnull(dp4nJumlahDigunakan,0) as digunakan,
	ifnull(dp4nJumlahLebihan,0) as lebihan
FROM	
	sdm_ref_dp4_penilaian
	INNER JOIN pub_ref_jabatan_fungsional ON jabfungrJenisrId=dp4refJabFungJenisrId
	INNER JOIN sdm_jabatan_fungsional ON jbtnJabfungrId=jabfungrId AND jbtnStatus='Aktif' AND jbtnPegKode='%s'
	LEFT JOIN sdm_dp4_penilaian ON dp4nDp4refKode=dp4refId AND dp4nDp4Id='%s'
	LEFT JOIN sdm_dp4 ON dp4nDp4Id=dp4Id AND dp4PegKode=jbtnPegKode
WHERE
	dp4refUnsur='%s' AND dp4refAktif='Ya'
