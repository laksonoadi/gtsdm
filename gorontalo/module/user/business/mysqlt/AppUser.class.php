...
class AppUser extends Database {
   ...
   function PernahLogin($username){
      $result = $this->Open($this->mSqlQueries['pernah_login'], array($username));
      if (($result[0]['login']=='0')||($result[0]['login']==0)){
	    $gantistatuslogin = $this->Execute($this->mSqlQueries['update_status_login'], array($result[0]['userId']));
		return false;
	  }else{
		return true;
	  }
   ...
   function cekValiditasOldPassword($pass,$id){
      //echo sprintf($this->mSqlQueries['get_data_user_by_password_and_id'],$pass,$id);
      $result = $this->Open($this->mSqlQueries['get_data_user_by_password_and_id'], array($pass,$id));
      return $result;
   }
...
//===DO==
   ...
   //OK 
   function DoUpdatePasswordWithUserName($password, $old_password, $username) {
      $result = $this->Execute($this->mSqlQueries['do_update_password_with_username'], array($password, $old_password, $username));
      return $result;
   }
