...
class ViewAdminListBerita extends HtmlResponse {
   ...
   function ProcessRequest() {
      $objectBerita = new ManejemenBerita();
      $status = $_GET['statusAdd'];
	  $status2 = $_GET['statusUpdate'];
	  $status3 = $_GET['statusDelete'];
	  //print_r($sukses);die;
	  if ($status == '1'){
      ...
      $count = $objectBerita->CountBerita();
      ...
      $data['list_berita'] = $objectBerita->ListBerita($offset,$limit);
      return $data;
   ...
   if($this->Pesan)
      ...
      if(!empty($data['list_berita'][0]['TITLE'])){
         ...
         foreach($data['list_berita'] as $berita){
            ...
            $this->mrTemplate->addVar('list_berita','DATE_POSTED',$berita['DATE_POSTED']);
