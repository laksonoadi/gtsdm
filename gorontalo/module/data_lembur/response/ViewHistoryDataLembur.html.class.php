...
class ViewHistoryDataLembur extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('approval', 'URL_APPROVAL_LEMBUR', Dispatcher::Instance()->GetUrl('approval_lembur', 'historyDataLembur', 'view', 'html') . "&dataId=" . $data['idPegawai']);
		...
		$this->mrTemplate->AddVar('content', 'URL_TAMBAH_LEMBUR', Dispatcher::Instance()->GetUrl('data_lembur', 'dataLembur', 'view', 'html') . "&op=add"."&dataId=" . $this->encDataId);
		...
		if (empty($data['dataHistory'])) {
			...
			for ($i=0; $i<sizeof($dataHistory); $i++) {
				...
				$dataHistory[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('data_lembur', 'deleteDataLembur', 'do', 'html') . '&dataId=' . $idEnc;
				$dataHistory[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('data_lembur', 'dataLembur', 'view', 'html') . '&op=edit' . '&dataId=' . $data['idPegawai'] . '&dataId2=' . $idEnc;
				$dataHistory[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('data_lembur', 'detailDataLembur', 'view', 'html') . '&dataId=' . $data['idPegawai'] . '&dataId2=' . $idEnc;
        
				if ($dataHistory[$i]['status']=='request'){
					$this->mrTemplate->SetAttribute('edit_data', 'visibility', 'visible');
					$dataHistory[$i]['tglstatus']='-';
				}else{
					$this->mrTemplate->SetAttribute('edit_data', 'visibility', 'hidden');
				}
				...
				if ($dataHistory[$i]['status']=='request') { $dataHistory[$i]['gambar']='lamp-off.gif'; }
