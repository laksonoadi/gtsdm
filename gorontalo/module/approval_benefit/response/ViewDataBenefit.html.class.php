...
class ViewDataBenefit extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/approval_benefit/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ProcessRequest()
   {
      ...
      if ($_GET['dataId'] != '') {
        ...
        $detailPegawai = $Obj->GetDetailPegawaiById($pilihpegawai);
        ...
        if($resultBenefit){
         ...
         $return['input']['tgl_status'] = $resultBenefit[0]['tgl_status'];
         $return['input']['user_id'] = $resultBenefit[0]['user_id'];
         $return['data']['klaim']=$resultKlaim;
         $return['input']['total_number'] = sizeof($resultKlaim);
      ...
      if($_GET['op'] == 'add'){
        ...
        //combo jenis benefit
        $jenisBenefit = $Obj->GetComboJenisBenefit();
        for ($i=0; $i<sizeof($jenisBenefit); $i++){
          if ($jenisBenefit[$i]['id']==$return['input']['id_benefit']){
              $return['input']['id_benefit_label']=$jenisBenefit[$i]['name'];
          }
        }
        
        //Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jenis_benefit', array('jenis_benefit',$jenisBenefit,$return['input']['id_benefit'],'false',' style="width:170px;"'), Messenger::CurrentRequest);
        ...
        //Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'relasi_pasien', array('relasi_pasien',$relasi,$return['input']['relasi_pasien'],'',' style="width:100px;"'), Messenger::CurrentRequest);
        ...
        Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'status', array('status',$status,$return['input']['status'],'',' style="width:100px;"'), Messenger::CurrentRequest);
        
        $tgl=explode('-',$return['input']['tgl_benefit']);
        $return['input']['tgl_benefit']=$this->periode2stringEng($return['input']['tgl_benefit']);
        $return['input']['tgl_benefit_day']=$tgl[2];
        $return['input']['tgl_benefit_mon']=$tgl[1];
        $return['input']['tgl_benefit_year']=$tgl[0];
        
        $tgl=explode('-',$return['input']['tgl_klaim']);
        $return['input']['tgl_klaim']=$this->periode2stringEng($return['input']['tgl_klaim']);
        $return['input']['tgl_klaim_day']=$tgl[2];
        $return['input']['tgl_klaim_mon']=$tgl[1];
        $return['input']['tgl_klaim_year']=$tgl[0];
        
        if($return['input']['tgl_status'] == '0000-00-00 00:00:00'){
          Messenger::Instance()->SendToComponent('tanggal', 'tanggal', 'view', 'html', 'tgl_stat', 
          array(date("Y-m-d"),'2003',$y1,'',''), Messenger::CurrentRequest);
        } else {
          Messenger::Instance()->SendToComponent('tanggal', 'tanggal', 'view', 'html', 'tgl_stat', 
          array($return['input']['tgl_status'],'2003',$y1,'',''), Messenger::CurrentRequest);
        }
      }
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('approval_benefit', 'historyDataBenefit', 'view', 'html') . '&dataId=' . $dataPegawai['id'].'&pilihpegawai=' . $_GET['pilihpegawai'].'&status='.$dat['status']);
      $this->mrTemplate->AddVar('content', 'URL_AKSI', Dispatcher::Instance()->GetUrl('approval_benefit', 'inputDataBenefit', 'do', 'html')); 
