...
class ViewHistoryDataBenefit extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/approval_benefit/'.GTFWConfiguration::GetValue('application', 'template_address').'');
	...
	//view
		...
		$totalData = $Obj->GetCountBenefit($pilihpegawai, $tampilkan);
		$itemViewed = 15;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$dataBalance = $Obj->GetBalanceBenefitLeft($pilihpegawai);
		$dataHistory = $Obj->GetDataBenefit($startRec, $itemViewed, $pilihpegawai, $tampilkan);
    //print_r($dataHistory);
    $url = Dispatcher::Instance()->GetUrl(Dispatcher::Instance()->mModule, Dispatcher::Instance()->mSubModule, Dispatcher::Instance()->mAction, Dispatcher::Instance()->mType . '&dataId=' . $this->encDataId . '&pilihpegawai=' . Dispatcher::Instance()->Encrypt($pilihpegawai). '&tampilkan=' . Dispatcher::Instance()->Encrypt($tampilkan) . '&cari=' . Dispatcher::Instance()->Encrypt(1));
	...
	function ParseTemplate($data = NULL) {
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('approval_benefit', 'historyDataBenefit', 'view', 'html') . "&dataId=" . $this->encDataId);
    ...
    if (empty($dataBalance)) {
      ...
      $this->mrTemplate->AddVar('content', 'URL_TAMBAH_BENEFIT', Dispatcher::Instance()->GetUrl('approval_benefit', 'dataBenefit', 'view', 'html') . "&dataId=" . $this->encDataId.'&op=add');  
		}
    ...
    //tampilkan history benefit
		if (empty($data['dataHistory'])) {
          ...
          $dataHistory[$i]['class_name'] = '';
				...
				$urlAccept = 'approval_benefit|deleteDataBenefit|do|html-dataId-'.$idEnc;
        $urlKembali = 'approval_benefit|historyDataBenefit|view|html';
        ...
        $dataHistory[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('approval_benefit', 'detailDataBenefit', 'view', 'html') . '&dataId=' . $data['idPegawai'] . '&dataId2=' . $idEnc. '&pilihpegawai=' . $data['pilihpegawai'];
        $dataHistory[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('approval_benefit', 'dataBenefit', 'view', 'html') . '&op=edit' . '&dataId=' . $data['idPegawai'] . '&dataId2=' . $idEnc. '&pilihpegawai=' . $data['pilihpegawai'];
