...
class ViewDataTaf extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/approval_taf/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ProcessRequest()
   {
      ...
      if ($_GET['dataId'] != '') {
        ...
        if($resultTaf){
         ...
         $return['input']['tipe_label'] = $resultTaf[0]['jnstafNama'];
         $return['input']['tgl_aju'] = $resultTaf[0]['tafTglPengajuan'];
         $return['input']['tgl_aju_label'] = $Obj->periode2stringEng($resultTaf[0]['tafTglPengajuan']);
         $return['input']['alasan'] = $resultTaf[0]['tafAlasan'];
         $return['input']['supv_status'] = $resultTaf[0]['tafStatusSpv'];
         $return['input']['supv_tanggal'] = $resultTaf[0]['tafTglStatusSpv'];
         $return['input']['hrd_status'] = $resultTaf[0]['tafStatusHRD'];
         $return['input']['hrd_tanggal'] = $resultTaf[0]['tafTglStatusHRD'];
         $return['input']['fin_status'] = $resultTaf[0]['tafStatusFin'];
         $return['input']['fin_tanggal'] = $resultTaf[0]['tafTglStatusFin'];
         $return['input']['total_hari'] = $resultTaf[0]['tafTotalHariKeseluruhan'];
         $return['input']['total_anggaran'] = $resultTaf[0]['tafTotalAnggaran'];
      ...
      if ($return['input']['supv_tanggal']=='0000-00-00'){
          $return['input']['supv_tanggal']=date("Y-m-d");
      }
      
      Messenger::Instance()->SendToComponent('tanggal', 'tanggal', 'view', 'html', 'tgl_status', array($return['input']['supv_tanggal'],'2003',$y1,'',''), Messenger::CurrentRequest);
      ...
      $status[0]['id']='Approved'; $status[0]['name']='Approved';
      $status[1]['id']='Not Approved'; $status[1]['name']='Rejected';
      Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'status', array('status',$status,$return['input']['supv_status'],'','  '), Messenger::CurrentRequest);
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_ITSELF', Dispatcher::Instance()->GetUrl('approval_taf', 'dataTaf', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'OP', $_GET['op']);
      $this->mrTemplate->AddVar('content', 'ID_PEG', $detailPegawai['id']);
      $this->mrTemplate->AddVar('content', 'PILIH_PEGAWAI', $data['pilihpegawai']);
      ...
      $this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('approval_taf', 'historyDataTaf', 'view', 'html') . '&dataId=' . $dataPegawai['id'].'&pilihpegawai=' . $data['pilihpegawai']);
      $this->mrTemplate->AddVar('content', 'URL_AKSI', Dispatcher::Instance()->GetUrl('approval_taf', 'inputDataTaf', 'do', 'html')); 
