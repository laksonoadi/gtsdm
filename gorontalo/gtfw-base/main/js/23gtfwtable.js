...
//dataJSONRest = '';
function parseData(data,url,notif,id){
	//dataJSONRest='';
	notif = (notif!=undefined)?notif:false;
	var idObject = (id!=undefined)?id:'subcontent-element';
	data = JSON.parse(data.responseText);
	if(data.gtfwResult){
		dataJson = data.gtfwResult;
		if(dataJson.status == '201' || dataJson.status == '419'){ 
			if(url==undefined||url==''){if(notif)showNotif(dataJson.message,dataJson.data);return dataJson.data;} //return data
		...
		}else if(notif) showNotif(dataJson.message,dataJson.data);
	}else if(notif)showNotif('Invalid Format','notebox-warning');
}
