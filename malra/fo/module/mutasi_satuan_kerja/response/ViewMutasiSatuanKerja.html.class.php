...
class ViewMutasiSatuanKerja extends HtmlResponse
   {
      ...
      $dataName = $dataJabs[$i]['jabstruk'];
         ...
         $dataJabs[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_satuan_kerja','MutasiSatuanKerja', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
         $dataJabs[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_satuan_kerja','detailMutasi', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
         $dataJabs[$i]['tanggal'] = $this->date2string($dataJabs[$i]['tmt']);
         $dataJabs[$i]['tgl_sk'] = $this->date2string($dataJabs[$i]['tgl_sk']);
