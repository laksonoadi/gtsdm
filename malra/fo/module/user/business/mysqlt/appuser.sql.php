...
$sql['pernah_login'] = "
   SELECT 
      peguserPernahLogin as login,
	  peguserUserId as userId
   FROM 
      pub_pegawai_user
      LEFT JOIN gtfw_user ON peguserUserId=userId
   WHERE
      UserName='%s'
";

$sql['update_status_login'] = "
   UPDATE pub_pegawai_user
   SET
      peguserPernahLogin=1
   WHERE
	  peguserUserId='%s'
";
...
//OK
$sql['get_data_user_by_password_and_id'] = "
   SELECT 
      u.UserId AS user_id,
      u.UserName AS user_name,
      u.RealName AS real_name,
   ...
   FROM
      ...
      LEFT JOIN gtfw_unit ON unitappUnitId = UnitId
   WHERE
      u.Password=md5('%s')AND u.UserName = '%s'
";
//OK
...
//OK
$sql['do_update_password_with_username'] = 
   "UPDATE gtfw_user
   SET 
      Password=md5('%s')
   WHERE 
      Password=md5('%s') AND UserName='%s'";
