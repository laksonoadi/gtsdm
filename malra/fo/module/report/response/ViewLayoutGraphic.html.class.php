...
class ViewLayoutGraphic extends HtmlResponse {

   function TemplateModule() {
      $this->SetTemplateBasedir($this->mrConfig->mApplication['docroot'] . 'module/report/template');
      $this->SetTemplateFile('view_layout_graphic.html');
   }
   ...
   function ProcessRequest() {
      ...
      $totalData = $rep->GetTotalGraphic($data['kunci']);
		$data['data'] = $rep->GetGraphicByNama($data['kunci'], $startRec, $startRec+$itemViewed);
   ...
   function ParseTemplate($data = NULL) {
     // $this->ButtonRendering();
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('report', 'layoutGraphic', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('report', 'addLayoutGraphic', 
         'view', 'html'));
      $this->mrTemplate->AddVar('content', 'KUNCI', $data['kunci']);

      if ($data['data'][0]['graphic_id']!='') {
         $this->mrTemplate->AddVar('data_query', 'QUERY', 'ADA');
   		for ($i=0;$i<sizeof($data['data']);$i++) {
   		   ...
   		   $idEnc = Dispatcher::Instance()->Encrypt($data['data'][$i]['graphic_id']);
            $url_delete = Dispatcher::Instance()->GetUrl('confirm', 'confirmDelete', 'do', 'html') . 
               '&id=' . $idEnc.'&message='.
               Dispatcher::Instance()->Encrypt('Anda akan menghapus layout grafik ').'&dataName='.
               Dispatcher::Instance()->Encrypt($data['data'][$i]['graphic_judul']).
               '&label='.Dispatcher::Instance()->Encrypt('Manajemen Layout Grafik').'&urlDelete='.
               Dispatcher::Instance()->Encrypt('report|deleteLayoutGraphic|do|html').'&urlReturn='.
               Dispatcher::Instance()->Encrypt('report|layoutGraphic|view|html');
   		   $this->mrTemplate->AddVar('list_query', 'URL_DELETE', $url_delete);
   		   $this->mrTemplate->AddVar('list_query', 'URL_EDIT', Dispatcher::Instance()->GetUrl('report', 'updateLayoutGraphic', 
               'view', 'html').'&graph_id='.$idEnc);
      ...
      $this->mrTemplate->addVar('body', 'navigation', '&gt; <a href="'.$url.'">Pengaturan Template</a> &gt; Layout Grafik');
