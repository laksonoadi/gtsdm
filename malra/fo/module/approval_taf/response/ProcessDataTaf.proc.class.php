...
class Process
{
   ...
   function __construct() {
    ...
    $this->pegawaiObj = new DataPegawai();
	  ...
	  $this->pageView = Dispatcher::Instance()->GetUrl('approval_taf', 'historyDataTaf', 'view', 'html').'&pilihpegawai='.$_POST['pilihpegawai'];
	  $this->pageHistory = Dispatcher::Instance()->GetUrl('approval_taf', 'historyDataTaf', 'view', 'html').'&pilihpegawai='.$_POST['pilihpegawai'];
    $this->decId = $_GET['dataId2']->Integer()->Raw();
    $this->decId2 = $_GET['dataId']->Integer()->Raw();
    $this->pilihpegawai = $_POST['pilihpegawai']->Integer()->Raw();
	...
	function SetPost($param){
      ...
      for ($i=0; $i<sizeof($this->POST['data']['travel']['tujuan']); $i++){
          ...
          for ($ii=0; $ii<sizeof($this->POST['data']['allowance'][$tujuan]['total']); $ii++){
              ...
              $nilai=$this->Obj->num_toprocess($this->POST['data']['allowance'][$tujuan]['nilai'][$ii]);
              $this->POST['data']['allowance'][$tujuan]['total'][$ii]=$total;
              $this->POST['data']['allowance'][$tujuan]['nilai'][$ii]=$nilai;
      ...
      $this->a=$this->POST['tgl_aju'];
      $this->b=$this->POST['tgl_status_year'].'-'.$this->POST['tgl_status_mon'].'-'.$this->POST['tgl_status_day'];
  }
  ...
  function Check (){
    ...
    if (isset($error)){
      ...
      Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', $msg, Messenger::NextRequest);
  ...
  function AddDataTaf(){
    ...
    $this->Obj->StartTrans();
    $result = $this->Obj->UpdateApprovalSupervisor($tafId,$this->POST['status'],$this->b);
          ...
          for ($ii=0; $ii<sizeof($this->POST['data']['allowance'][$tujuan]['total']); $ii++){
               ...
               if (empty($id)){
                   ...
                   $array1=array('taf_id'=>$tafId,
                    ...
                    $result2 = $this->Obj->UpdateAnggaranApproval($array1);
               }
          }
      }
	...
	function InputDataTaf(){
      ...
      if ((isset($this->POST['btnsimpan'])) && ($this->POST['op'] == 'add')) {
        ...
        /*if ($rs_add==true){
          ...
          $arrBody[2]['replace']='{TAF_TIPE}'; $arrBody[2]['with']=$this->Obj->GetJenisTafById($_POST['jenis_taf']);
          $arrBody[3]['replace']='{TAF_TGL_AJU}'; $arrBody[3]['with']=$this->ObjEmail->IndonesianDate($this->a,'YYYY-MM-DD');
          $arrBody[4]['replace']='{TAF_TOTAL_HARI}'; $arrBody[4]['with']=$_POST['total_hari_keseluruhan'];
          $arrBody[5]['replace']='{TAF_TOTAL_ANGGARAN}'; $arrBody[5]['with']=$this->POST['total_budget'];
          $arrBody[6]['replace']='{TAF_ALASAN}'; $arrBody[6]['with']=$_POST['alasan'];
          $arrBody[7]['replace']='{TAF_CURRENCY}'; $arrBody[7]['with']=$this->Obj->GetCurrByJenisTafId($_POST['jenis_taf']);
          
          $body=$this->ObjEmail->getBodyEmailTafRequest($arrBody);
          $subject=$this->ObjEmail->getSubjectEmailTafRequest();
        ...
        $return .= "&dataId=".$this->POST['idPeg'];
        if($rs_add == true){
           Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgAddSuccess."<br/>".$kirim,$this->cssDone),Messenger::NextRequest);
           return $return;
        }else{
           Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgAddFail."<br/>".$kirim,$this->cssFail),Messenger::NextRequest);
           return $return;
        }
        ...
        if ($rs_update==true){
          ...
          $arrBody[9]['replace']='{TAF_STATUS}'; $arrBody[9]['with']=$_POST['status'];
          
          
          $body=$this->ObjEmail->getBodyEmail('email_taf_approval',$arrBody);
          $subject=$this->ObjEmail->getSubjectEmail('email_taf_approval');
          $kirim=$this->ObjEmail->kirimEmail($to,$cc,$bcc,$from,$subject,$body);
          //Akhir Kirim Email
        }
        ...
        if($rs_update == true){
           Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgUpdateSuccess."<br/>".$kirim,$this->cssDone),Messenger::NextRequest);
           return $return;
        }else{
           Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgUpdateFail."<br/>".$kirim,$this->cssFail),Messenger::NextRequest);
           return $return;
        }
      }
   }
   ...
   function Delete(){
    ...
    /*if ($deleteData==true){
          ...
          $body=$this->ObjEmail->getBodyEmailTafRequestCancel($arrBody);
          $subject=$this->ObjEmail->getSubjectEmailTafRequestCancel();
          $kirim=$this->ObjEmail->kirimEmail($to,$cc,$bcc,$from,$subject,$body);
          //Akhir Kirim Email
    }*/
    
    if($deleteData == true) {
			Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgDeleteSuccess."<br/>".$kirim, $this->cssDone),Messenger::NextRequest);   
		}else{
			Messenger::Instance()->Send('approval_taf', 'historyDataTaf', 'view', 'html', array($this->POST,$this->msgDeleteFail."<br/>".$kirim, $this->cssFail),Messenger::NextRequest);   
		}
    $return = $this->pageView;
