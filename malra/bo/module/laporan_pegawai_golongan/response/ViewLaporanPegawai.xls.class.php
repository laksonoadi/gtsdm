...
class ViewLaporanPegawai extends XlsResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=$totalKolomGolongan+4;
  		$row++;
		  $this->mWorksheets['Pegawai']->write($row, 0, 'LAPORAN JUMLAH PEGAWAI BERDASARKAN GOLONGAN', $this->fH1);
      ...
      array_push($header[0],'');
		  ...
		  $header[1]=array('','');
		  for ($i=0; $i<$totalKolomGolongan; $i++){
		    array_push($header[1],$this->ComboPangkatGolongan[$i]['name']);
		    array_push($header[1],'');
      }
      ...
      $header[3]=array(' ','Jenis Kelamin');
		  ...
		  for ($i=0; $i<$totalKolomGolongan; $i++){
		    array_push($val,$this->ComboPangkatGolongan[$i]['id'].'_l');
		    array_push($val,$this->ComboPangkatGolongan[$i]['id'].'_p');
      }
          ...
          $row++;
            ...
            $this->mWorksheets['Pegawai']->write($row, $i, '', $this->fH3);
    			...
    			for ($i=0; $i<sizeof($this->ComboUnitKerja); $i++) {
                 ...
                 $dataUnit[$i][$this->ComboPangkatGolongan[$ii]['id'].'_l']=$tempDataL;
                 $dataUnit[$i][$this->ComboPangkatGolongan[$ii]['id'].'_p']=$tempDataP;
              }
