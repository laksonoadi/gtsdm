...
class ViewGtfwModule extends HtmlResponse
{
   ...
   function ProcessRequest()
   {
	  ...
	  if ($GtfwModule->aktif=='bo'){
		...
		$path_module=str_replace("bo","fo",GTFWConfiguration::GetValue( 'application', 'docroot')).'module/';
   ...
   function ParseTemplate($data = NULL)
   {
     ...
     $this->mrTemplate->AddVar('content', 'LABEL_ACTION', isset($_GET['dataId']) ? 'Update' : 'Add');
	  ...
	  $this->mrTemplate->AddVar('content', 'URL_CARI', Dispatcher::Instance()->GetUrl('gtfw_module','gtfwModule','view','html'));//print_r($data['link']['url_action']);
