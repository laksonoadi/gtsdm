...
class ViewTanggal extends HtmlResponse {   
   ...
   function ParseTemplate($data = NULL) {                                                
      ...
      if(strtotime($data['selectedval']) === FALSE && $this->mComponentParameters['nullable'] == TRUE) {
         // $data['selectedval'] = '0000-00-00'; // Let it bee~
      } else {
         $data['selectedval'] = date('Y-m-d', strtotime($data['selectedval']));
      }
      ...
      if(is_array($status)):         
         $this->mrTemplate->addVar('content', 'STATUS_TANGGAL', $status[2]);
         $this->mrTemplate->addVar('content', 'STATUS_BULAN', $status[1]);
         $this->mrTemplate->addVar('content', 'STATUS_TAHUN', $status[0]);   
      else:
