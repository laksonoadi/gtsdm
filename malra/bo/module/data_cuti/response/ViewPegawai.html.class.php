...
class ViewPegawai extends HtmlResponse{
	...
	function ParseTemplate($data = NULL){
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('data_cuti', 'pegawai', 'view', 'html') );
		...
		if (empty($data['dataPegawai'])) {
			...
			$total=0;
  			for ($i=0; $i<sizeof($dataPegawai); $i++) {
				...
				$dataPegawai[$i]['URL_HISTORY'] = Dispatcher::Instance()->GetUrl('data_cuti','dataCuti', 'view', 'html').'&dataId='. $idEnc;
				$dataPegawai[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('data_cuti','historyDataCuti', 'view', 'html').'&dataId='. $idEnc;
