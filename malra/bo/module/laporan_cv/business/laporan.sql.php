...
SELECT
  ...
  h.statrPegawai AS statr,                               
    ...
    concat(bb.pendNama,' ',pddkJurusan,' ',pddkInstitusi) as pendidikan_tertinggi,
  	...
  	jabfung.jbtnTglMulai as jabatan_fungsional_tmt,
  	satkerId as unit_kerja_id,
  	satkerNama as unit_kerja,
    jk.nama_ref_jns_peg AS jenis_kepegawaian,
    jabstrukrNama as jabatan_struktural,
    jabstruk.jbtnTglMulai as jabatan_struktural_tmt,
    nameEselon as eselon,
...
FROM 
   pub_pegawai a
    ...
    LEFT JOIN 
  	...
  	LEFT JOIN sdm_jabatan_fungsional jabfung ON jabfung.jbtnPegKode=a.pegId AND jabfung.jbtnStatus='Aktif'
    ...
    LEFT JOIN sdm_ref_jenis_kepegawaian jk ON satkerpegJenPegId=jk.id_ref_jns_peg
    LEFT JOIN sdm_jabatan_struktural jabstruk ON jabstruk.jbtnPegKode=a.pegId AND jabstruk.jbtnStatus='Aktif'
    LEFT JOIN sdm_ref_jabatan_struktural ON jbtnJabstrukrId=jabstrukrId
    LEFT JOIN sdm_ref_eselon ON jbtnEselon=idEselon
WHERE
   a.pegId = '%s'
GROUP BY a.pegId
"; 
