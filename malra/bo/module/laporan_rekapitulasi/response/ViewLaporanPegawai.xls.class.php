...
class ViewLaporanPegawai extends XlsResponse{
	...
	function ProcessRequest(){
		...
		$this->kriteria = implode(', ', $this->kriteria);
		...
		$row++;
		$this->mWorksheets['Rekapitulasi']->write($row, 0, strtoupper('PER TANGGAL '.$this->tanggal), $this->fH1);
    	$this->mWorksheets['Rekapitulasi']->merge_cells($row, 0, $row,$jumKolom-1);
		$row++;
		$this->mWorksheets['Rekapitulasi']->write($row, 0, strtoupper('DENGAN KRITERIA '.$this->kriteria), $this->fH1);
    	$this->mWorksheets['Rekapitulasi']->merge_cells($row, 0, $row,$jumKolom-1);
