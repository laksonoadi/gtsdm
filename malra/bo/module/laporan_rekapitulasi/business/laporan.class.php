...
class Laporan extends Database {
	...
	function getVariabelGlobal(){		
		...
		$this->query_filter=array(
						'unit'=> $this->GetUnitListData($this->filter['unit']),
						// 'unit'=> 'AND satkerId=268 AND satkerLevel LIKE "1.7.1%"',
...
//==GET== 
	function GetComboUnitKerja($combo=false){
		...
		$result=$this->Open($sql,array($satker, $satlev));
		// print_r($sql);
	...
	function GetUnitListData($unit){
          $result = $this->Open($this->mSqlQueries['get_satker_by_id'], array($unit));
	      if($result){
            $satker = $result[0]['satkerId'];
            $satlev = $result[0]['satkerLevel'];
	      } else {
          ...
          $resultdata = " AND (satkerId = '".$satker."' OR satkerLevel LIKE CONCAT('".$satlev."', '.%%')) ";
		/* $sql=$this->mSqlQueries['get_unit_list'];
		// print_r($sql);
		$result=$this->Open($sql,array($unit));
		
		if($result){
			$result = $result['0'];
			$resultlist = $this->GetUnitListDataAll($result['unit_id']);
			// print_r($resultlist);
		$resultdata =  " AND satkerId IN('".$unit."','".$resultlist."')";
	}else{
		$resultdata =  'AND 1=1';
	}
	// print_r($resultdata);
	...
	function GetUnitListDataAll($unit){
		$sql=$this->mSqlQueries['get_unit_list_data'];
		// print_r($sql);
		$unit = $unit.'.%';
		$result=$this->Open($sql,array($unit));
	...
	// print_r($resultdata);
		if($result){
		   $unitgrup = '';
	     foreach ($result as $key => $value) {
	        $unitgrup .= $value['unit_id'].',';
	      }
	      $uGroupunit = str_replace(",","','",$unitgrup);
	        $resultunitgroup = $uGroupunit.'0';
	      
	      // print_r($resultunitgroup);
	      return $resultunitgroup;
		}else{
		return $result;	
		}
