...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_nominatif/business/laporan.sql.php';
...
//==GET== 
   function GetComboUnitKerja(){
    ...
    public function GetCountDataNominatif()
    {
        $query = $this->mSqlQueries['count_data_nominatif'];
        $result = $this->Open($query, array());
        return $result[0]['total'];
    }
	 
   /* function GetCountDataNominatif($unit_kerja,$jenis_pegawai = 'CPNS') {
      $sql='SELECT COUNT(*) AS total FROM (' . $this->mSqlQueries['get_data_nominatif'] . ') t'; 
      ...
      if($unit_kerja != "all"){
  	  ...
  	  $sql=str_replace('%jenis_pegawai%',' AND jnspegrNama IN (\''.$jenis_pegawai.'\') ' ,$sql);
   ...
   function GetDataNominatif($offset, $limit, $unit_kerja, $jenis_pegawai = 'CPNS') {
      $sql=$this->mSqlQueries['get_data_nominatif']; 
      $result=$this->Obj->GetSatkerDetail($unit_kerja);
      if($result){
          $satker = $result['satkerId'];
          $satlev = $result['satkerLevel'];
      } else {
          $satker = 0;
          $satlev = 0;
      }
      if($unit_kerja != "all"){
        ...
        $sql=str_replace('%user_unit_kerja%', " AND (subunit.satkerLevel LIKE CONCAT('".$satkerlevel."', '.%%') OR subunit.satkerId = '".$satker."') ", $sql);
	...
	function GetComboVariabel2(){
		$sql=$this->mSqlQueries['get_combo_jenis'];
		$result=$this->Open($sql,array());
		return $result;
	}
