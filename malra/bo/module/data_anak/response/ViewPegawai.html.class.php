...
class ViewPegawai extends HtmlResponse
{
   ...
   function ProcessRequest()
   {
	...
	function ParseTemplate($data = NULL){
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('data_anak', 'pegawai', 'view', 'html') );
		...
		if (empty($data['dataPegawai'])) {
			...
			$total=0;
  			for ($i=0; $i<sizeof($dataPegawai); $i++) {
				...
				$dataPegawai[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('data_anak','dataAnak', 'view', 'html').'&dataId='. $idEnc;
