...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_duk/business/laporan.sql.php';
...
//==GET==    
   ...
   function GetComboJenisKelamin(){
      $return[0]['id']='L'; $return[0]['name']='Laki-laki';
      $return[1]['id']='P'; $return[1]['name']='Perempuan'; 
      return $return;
   }
   
   function GetComboJenisPegawai(){
      return $this->Open($this->mSqlQueries['get_combo_jenis_pegawai'],array());
   }
   
   function GetCountDataDuk($unit_kerja, $pangkat_golongan, $jenis_kelamin, $jenis_pegawai) {
      $sql='SELECT COUNT(*) AS total FROM (' . $this->mSqlQueries['get_data_duk'] . ') t'; 
     ...
     foreach ($result as $key => $value) {
      ...
      if($jenis_pegawai != "all"){
  		  $sql=str_replace('%jenis_pegawai%',' AND jnspegrId='.$jenis_pegawai ,$sql);
      }else{
        $sql=str_replace('%jenis_pegawai%','',$sql);
      }
      
      if($jenis_kelamin != "all"){
  		  $sql=str_replace('%jenis_kelamin%'," AND pegKelamin='".$jenis_kelamin."'" ,$sql);
      }else{
        $sql=str_replace('%jenis_kelamin%','',$sql);
      }
   ...
   function GetDataDuk($offset, $limit, $unit_kerja, $pangkat_golongan, $jenis_kelamin, $jenis_pegawai) {
      $sql=$this->mSqlQueries['get_data_duk']; 
