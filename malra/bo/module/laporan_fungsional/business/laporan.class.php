...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_fungsional/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetComboStatusJabatan(){
      $result[0]['id']='Aktif'; $result[0]['name']='Aktif';
      $result[1]['id']='Tidak Aktif'; $result[1]['name']='Tidak Aktif';
      return $result;
   }
   /* 
   function GetCountDataFungsional($unit_kerja, $jabatan_fungsional, $status_jabatan) {
      $sql=$this->mSqlQueries['count_data_fungsional']; 
      $list=$this->Obj->GetListIdAnakUnitByUnitId($unit_kerja);
      if($unit_kerja != "all"){
  		  $sql=str_replace('%unit_kerja%',' AND satkerId IN ('.$list.')' ,$sql);
      }else{
        $sql=str_replace('%unit_kerja%','',$sql);
      }
      
      if($jabatan_fungsional != "all"){
  		  $sql=str_replace('%jabatan_fungsional%'," AND jabfungrJenisrId='".$jabatan_fungsional."'",$sql);
      }else{
        $sql=str_replace('%jabatan_fungsional%','',$sql);
      }
      
      if($status_jabatan != "all"){
  		  $sql=str_replace('%status_jabatan%'," AND jbtnStatus='".$status_jabatan."'",$sql);
      }else{
        $sql=str_replace('%status_jabatan%','',$sql);
      }
    ...
    public function GetCountDataFungsional()
    {
        $query = $this->mSqlQueries['count_data_fungsional'];
        $result = $this->Open($query, array());
        return $result[0]['total'];
    }
   
   function GetDataFungsional($offset, $limit, $unit_kerja, $jabatan_fungsional, $status_jabatan) {
      $sql=$this->mSqlQueries['get_data_fungsional']; 
      $result=$this->Obj->GetSatkerDetail($unit_kerja);
      if($result){
      ...
      if($status_jabatan != "all"){
        ...
        $result = $this->Obj->GetSatkerAndLevel();
        if($result) {
            $satker = $result[0]['satkerId'];
      ...
      $result=$this->Open($sql, array($satlev, $satker));
