...
class ViewMutasiPak extends HtmlResponse
	{
		...
		function ParseTemplate($data = NULL)
		{
			...
			if($data['lang']=='eng') {
				...
				$this->mrTemplate->AddVar('content', 'LABEL_ACTION', isset($_GET['dataId']) ? 'Approval' : 'Add');
				...
				$this->mrTemplate->AddVar('content', 'LABEL_ACTION', isset($_GET['dataId']) ? 'Persetujuan' : 'Tambah');
				$this->mrTemplate->AddVar('content', 'BUTTON', isset($_GET['dataId']) ? ' Batal ' : 'Reset');
				$this->mrTemplate->AddVar('content', 'TYPE', isset($_GET['dataId']) ? 'submit' : 'reset');
			}
	  
			if ( isset($_GET['dataId'])) {
				$this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan', 'updateMutasiPak', 'do', 'html'));
			}else{
				$this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan', 'addMutasiPak', 'do', 'html'));
			}
      
			$this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan', 'Pegawai', 'view', 'html') );
			...
			if (empty($dataPak)) {
				...
				$urlDelete = Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan', 'deleteMutasiPak', 'do', 'html');
				$urlReturn = Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan', 'MutasiPak', 'view', 'html');
				...
				for ($i=0; $i<count($dataPak); $i++) {
					...
					$urlAccept = 'mutasi_pak_penetapan|deleteMutasiPak|do|html-id-'.$dataPegawai[0]['id'];
					$urlKembali = 'mutasi_pak_penetapan|MutasiPak|view|html-id-'.$dataPegawai[0]['id'];
					...
					$dataPak[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan','MutasiPak', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
					$dataPak[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_pak_penetapan','detailMutasi', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
					$dataPak[$i]['URL_DETAIL_KEGIATAN'] = Dispatcher::Instance()->GetUrl('mutasi_pak_kumulatif','detailMutasi', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
					if (($dataPak[$i]['ditetapkan']=='0')||($dataPak[$i]['ditetapkan']==0)){
						$dataPak[$i]['EDIT']='<a class="xhr dest_subcontent-element" href="'.$dataPak[$i]['URL_EDIT'].'" title="Persetujuan"><img src="images/button-check.gif" alt="Persetujuan"/></a>';
