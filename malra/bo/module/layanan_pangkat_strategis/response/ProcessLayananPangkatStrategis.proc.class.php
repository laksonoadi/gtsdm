...
class Process {
	...
	public function __construct() {
		...
		$this->pageView = array('layanan_pangkat_strategis','layananPangkatStrategis','view','html');
		$this->pageInput = array('layanan_pangkat_strategis','inputLayananPangkatStrategis','view','html');
	}
	...
	function update() {
		...
		$param=array(
			...
			$detail['id_pend'],
			$detail['id_old_jabstruk'],
			$detail['id_old_jabfung'],
			$detail['id_satker'],
			$this->_POST['start_sk'],
	...
	public function add() {
        ...
        if(isset($this->_POST['issue_date_year']) && isset($this->_POST['issue_date_mon']) && isset($this->_POST['issue_date_day']))
            $this->_POST['issue_date'] = $this->_POST['issue_date_year'].'-'.$this->_POST['issue_date_mon'].'-'.$this->_POST['issue_date_day'];
        $old_jabstruk_id = (!empty($pegawai['old_jabstruk_id']) ? $pegawai['old_jabstruk_id'] : NULL);
        $old_jabfung_id = (!empty($pegawai['old_jabstruk_id']) ? NULL : $pegawai['old_jabfung_id']);
