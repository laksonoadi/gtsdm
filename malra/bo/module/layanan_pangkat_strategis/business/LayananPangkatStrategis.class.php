...
class LayananPangkatStrategis extends Database {

   protected $mSqlFile= 'module/layanan_pangkat_strategis/business/layananpangkatstrategis.sql.php';
...
//==GET==    
   ...
   function GetCountListSkPangkat($unit_kerja, $pangkat_golongan, $status, $awal, $akhir) {
      $sql='SELECT COUNT(*) AS total FROM (' . $this->mSqlQueries['get_list_pangkat_strategis'] . ') t';
      ...
      if($status != "all" && $status != "") {
        if($status == '1') {
            $sql = str_replace('%status1%', " INNER JOIN sdm_sk_pangkat_strategis ON pktgolId = id_pktgol ", $sql);
            $sql = str_replace('%status2%', '', $sql);
        } else {
            $sql = str_replace('%status1%', " LEFT JOIN sdm_sk_pangkat_strategis ON pktgolId = id_pktgol ", $sql);
   ...
   function GetListSkPangkat($offset, $limit, $unit_kerja, $pangkat_golongan, $status, $awal, $akhir) {
      $sql=$this->mSqlQueries['get_list_pangkat_strategis'];
   ...
   function CountSkPangkatByPktgolId($id) {
       $result = $this->Open($this->mSqlQueries['count_sk_pangkat_strategis_by_pktgol_id'], array($id));
       if(!$result) return FALSE;
       return $result[0]['total'];
   }
   
   function GetSkPangkatByPktgolId($id) {
       $query = $this->mSqlQueries['get_sk_pangkat_strategis'];
