...
class ViewLaporanPegawai extends XlsResponse
{
   ...
   function ProcessRequest()
   {
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=$totalKolomStruktural+4;
  		$row++;
		  $this->mWorksheets['Pegawai']->write($row, 0, 'LAPORAN JUMLAH PEGAWAI BERDASARKAN JABATAN STRUKTURAL', $this->fH1);
    	$this->mWorksheets['Pegawai']->merge_cells($row, 0, $row,$jumKolom-1);
		  ...
		  for ($i=0; $i<$totalKolomStruktural; $i++){
		    if ($i==0){
		      array_push($header[0],'Struktural');
		      array_push($header[0],'');
        }else{
          array_push($header[0],'');
		      array_push($header[0],'');
      ...
      array_push($header[0],'');
		  ...
		  $header[1]=array('','');
		  for ($i=0; $i<$totalKolomStruktural; $i++){
		    array_push($header[1],$this->ComboStruktural[$i]['name']);
		    array_push($header[1],'');
      }
      ...
      $header[3]=array(' ','Jenis Kelamin');
		  ...
		  for ($i=0; $i<$totalKolomStruktural; $i++){
		    array_push($val,$this->ComboStruktural[$i]['id'].'_l');
		    array_push($val,$this->ComboStruktural[$i]['id'].'_p');
      }
          ...
          $row++;
            ...
            $this->mWorksheets['Pegawai']->write($row, $i, '', $this->fH3);
    			...
    			for ($i=0; $i<sizeof($this->ComboUnitKerja); $i++) {
                 ...
                 $dataUnit[$i][$this->ComboStruktural[$ii]['id'].'_l']=$tempDataL;
                 $dataUnit[$i][$this->ComboStruktural[$ii]['id'].'_p']=$tempDataP;
              }
