...
class LayananPangkatFungsional extends Database {

   protected $mSqlFile= 'module/layanan_pangkat_fungsional/business/layananpangkatfungsional.sql.php';
...
//==GET==    
   ...
   function GetCountListSkPangkat($unit_kerja, $pangkat_golongan, $status, $awal, $akhir) {
      $sql='SELECT COUNT(*) AS total FROM (' . $this->mSqlQueries['get_list_pangkat_fungsional'] . ') t';
      ...
      if($unit_kerja == "all"){
        // All; use user's data
        $sql=str_replace('%unit_kerja%', ' AND (subunit.satkerId="'.$satker.'" OR subunit.satkerLevel LIKE CONCAT("'.$satlev.'",".%")) ', $sql);
      } elseif($unit_kerja != ""){
        $sql=str_replace('%unit_kerja%', ' AND (subunit.satkerId='.$unit_kerja.' OR subunit.satkerLevel LIKE CONCAT((SELECT satkerLevel FROM pub_satuan_kerja WHERE satkerId='.$unit_kerja.'),".%")) ', $sql);
      } else {
  		  // $sql=str_replace('%unit_kerja%', ' AND (subunit.satkerId='.$satker.' OR subunit.satkerLevel LIKE CONCAT("'.$satlev.'",".%")) ' ,$sql);
  		  $sql=str_replace('%unit_kerja%', ' AND subunit.satkerId="-1" ' ,$sql); // Prevent search if haven't chosen
      }
      ...
      if($status != "all" && $status != "") {
        if($status == '1') {
            $sql = str_replace('%status1%', " INNER JOIN sdm_sk_pangkat_fungsional ON pktgolId = id_pktgol ", $sql);
            $sql = str_replace('%status2%', '', $sql);
        } else {
            $sql = str_replace('%status1%', " LEFT JOIN sdm_sk_pangkat_fungsional ON pktgolId = id_pktgol ", $sql);
            $sql = str_replace('%status2%', " AND id_pktgol IS NULL ", $sql);
        }
      } else {
        $sql = str_replace('%status1%', " LEFT JOIN sdm_sk_pangkat_fungsional ON pktgolId = id_pktgol ", $sql);
        $sql = str_replace('%status2%', '', $sql);
      ...
      if($awal != "" && $akhir != ""){
        $sql=str_replace('%awal_akhir%', sprintf(" AND (pktgolNaikPktYad BETWEEN '%s' AND '%s') ", $awal, $akhir), $sql);
      }else{
   ...
   function GetListSkPangkat($offset, $limit, $unit_kerja, $pangkat_golongan, $status, $awal, $akhir) {
      $sql=$this->mSqlQueries['get_list_pangkat_fungsional'];
   ...
   function GetPangkatPegawai($id) {
        $sql = $this->mSqlQueries['get_pangkat_pegawai']; 
   ...
   function CountSkPangkatByPktgolId($id) {
       $result = $this->Open($this->mSqlQueries['count_sk_pangkat_fungsional_by_pktgol_id'], array($id));
       if(!$result) return FALSE;
       return $result[0]['total'];
   }
   
   function GetSkPangkatByPktgolId($id) {
       $query = $this->mSqlQueries['get_sk_pangkat_fungsional'];
       
       $where = " AND id_pktgol = '%s' ";
       $query = str_replace('--where--', $where, $query);
       $result = $this->Open($query, array($id));
       return $result[0];
   }
   
   function GetSkPangkatById($id) {
       $query = $this->mSqlQueries['get_sk_pangkat_fungsional'];
