...
class ViewMutasiKgb extends HtmlResponse
   {
      ...
      $this->profilId=$id;
         ...
         $dataKgb = $kgb->GetListMutasiKgb($id);
               ...
               $result=$dataMutasi[0];
                  if(!empty($result)){
                     ...
                     $return['input']['nip'] = $result['nip'];
			            ...
			            $idKomp = $kgb->GetIdKomp($result['pktgolid'],$result['masa_thn']);
			            $result2=$idKomp[0];
                  if(!empty($result2)){
            ...
            $return['input']['gapok'] = '';//$dataPegawai[0]['next_gaji'];
            $return['input']['masa_thn'] = '';
            $return['input']['masa_bln'] = '';
            $return['input']['mulai'] = date("Y-m-d");
            $date = date("Y-m-d");
            $return['input']['yad'] = date("Y-m-d",strtotime("+24 months $date"));
         ...
         $arrpktgol = $kgb->GetComboPangkatGolonganAll();
