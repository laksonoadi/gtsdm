...
class ViewPegawai extends HtmlResponse {
  ...
  function ProcessRequest() {
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('mutasi_penelitian', 'Pegawai', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'TITLE', 'MUTASI PENELITIAN');
      ...
      if (empty($data['mpg'])) {
         $this->mrTemplate->AddVar('data_mutasi_penelitian', 'DATA_EMPTY', 'YES');
      } else {      
         $this->mrTemplate->AddVar('data_mutasi_penelitian', 'DATA_EMPTY', 'NO');
         $len = sizeof($data['mpg']);
         foreach ($data['mpg'] as $key => $value) {
            ...
            //$urlAccept = 'mutasi_penelitian|deletePangkatGolongan|do|html';
            //$urlKembali = 'pangkat_golongan|PangkatGolongan|view|html';
            
            $value['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_penelitian','MutasiPenelitian', 'view', 'html').'&id='.Dispatcher::Instance()->Encrypt($value['id']);
            $this->mrTemplate->AddVars('data_mutasi_penelitian_item', $value, 'MPG_');
            $this->mrTemplate->parseTemplate('data_mutasi_penelitian_item', 'a');
         }
      }
   }
}
