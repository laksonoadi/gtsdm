<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot') . 'module/persetujuan_benefit/business/app_benefit.class.php';

class ViewAppDataBenefit extends HtmlResponse {

	var $Pesan;

	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/persetujuan_benefit/'.GTFWConfiguration::GetValue('application', 'template_address').'');
		$this->SetTemplateFile('view_app_data_benefit.html');
	}
	...
	//view
    $datenow = date("Y-m-d");
		$totalData = $Obj->GetCountBenefit($nip_nama);
		$itemViewed = 15;
		$currPage = 1;
		$startRec = 0 ;
		if(isset($_GET['page'])) {
		...
		$dataHistory = $Obj->GetDataAppBenefit($nip_nama, $startRec, $itemViewed);
	...
	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('persetujuan_benefit', 'appDataBenefit', 'view', 'html'));
		if($this->Pesan) {
        ...
        $no = $i+$data['start'];
          ...
          $dataHistory[$i]['class_name'] = '';
				...
				$idEnc2 = Dispatcher::Instance()->Encrypt($dataHistory[$i]['peg_id']);
				
        $dataHistory[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('persetujuan_benefit', 'deleteDataBenefit', 'view', 'html') . '&dataId=' . $idEnc;
        $dataHistory[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('persetujuan_benefit', 'inputAppBenefit', 'view', 'html') . '&dataId=' . $idEnc2 . '&dataId2=' . $idEnc;
        
        $dataHistory[$i]['tgl_submit'] = $this->periode2string($dataHistory[$i]['tgl_submit']);
