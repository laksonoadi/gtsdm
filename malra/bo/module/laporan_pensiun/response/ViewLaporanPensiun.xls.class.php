<?php
require_once GTFWConfiguration::GetValue( 'application', 'docroot').'module/laporan_pensiun/business/laporan.class.php';
   
class ViewLaporanPensiun extends XlsResponse
{
   var $mWorksheets = array('Pensiun');
   
   function GetFileName() {
      // name it whatever you want
      return 'Laporan_Pensiun_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      ...
      if(isset($_POST['awal_year'])) {
		  ...
		  $totalData = $this->Obj->GetCountDataPensiun($this->awal,$this->akhir,$this->unit_kerja, $this->pangkat_golongan);
		  $dataPegawai = $this->Obj->GetDataPensiun(0, $totalData, $this->awal,$this->akhir, $this->unit_kerja, $this->pangkat_golongan);
  		$data=$dataPegawai;
  		$row=-1;
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=11;
  		...
  		$row++;
		  $this->mWorksheets['Pensiun']->write($row, 0, 'DATA PEGAWAI (YANG AKAN) PENSIUN', $this->fH1);
    	$this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
    	
    	$row++;
		  $this->mWorksheets['Pensiun']->write($row, 0, 'Periode Awal : '.$this->label_awal , $this->fH2);
		  $this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Pensiun']->write($row, 0, 'Periode Akhir : '.$this->label_akhir , $this->fH2);
		  $this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Pensiun']->write($row, 0, 'Unit Kerja  : '.$this->label_unit_kerja , $this->fH2);
		  $this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  $row++;
		  $this->mWorksheets['Pensiun']->write($row, 0, 'Pangkat/Golongan  : '.$this->label_pangkat_golongan , $this->fH2);
		  $this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
		  
		  //Set Header
		  $header[0]=array('No','NIP','Nama','Pangkat','Jabatan Struktural','Jabatan Fungsional','Tanggal Lahir','Usia','Usian Pensiun','Tanggal Pensiun','Unit Kerja');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','nip','nama','pangkat','jabatan_struktural','jabatan_fungsional','tanggal_lahir','umur','usia_pensiun','tanggal_pensiun','unit_kerja');
      ...
      for ($i=0; $i<sizeof($header); $i++){
        ...
        for ($ii=0; $ii<sizeof($val); $ii++){
          $this->mWorksheets['Pensiun']->write($row, $ii, $header[$i][$ii], $this->fH3);
		  ...
		  for ($i=1; $i<=sizeof($merger); $i++){
		    $this->mWorksheets['Pensiun']->merge_cells($merger[$i]['row_awal'], $merger[$i]['col_awal'], $merger[$i]['row_akhir'], $merger[$i]['col_akhir']);
      }
      
      $row++;
      for ($i=0; $i<$jumKolom; $i++){
        $this->mWorksheets['Pensiun']->write($row, $i, $i+1, $this->fH3);
      }
      
      if (sizeof($data)<=0) {
          $row++;
    			$this->mWorksheets['Pensiun']->write($row, 0, 'Data Tidak Ditemukan', $this->fH3);
    			for ($i=1; $i<$jumKolom; $i++){
            $this->mWorksheets['Pensiun']->write($row, $i, '', $this->fH3);
          }
    			$this->mWorksheets['Pensiun']->merge_cells($row, 0, $row,$jumKolom-1);
    	} else {
    			$dataPegawai = $data;
              ...
              $dataPegawai[$i]['nip'] = $dataPegawai[$i]['nip'].' ';
        		  ...
        		  $dataPegawai[$i]['tanggal_pensiun']=$this->Obj->IndonesianDate($dataPegawai[$i]['tanggal_pensiun'],'YYYY-MM-DD');
    					for ($ii=0; $ii<sizeof($val); $ii++){
					 if($val[$ii] == 'nip') {
						$this->mWorksheets['Pensiun']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData3);
					 } else {
						$this->mWorksheets['Pensiun']->write($row, $ii, $dataPegawai[$i][$val[$ii]], $this->fColData2);
					 }
            ...
            $this->mWorksheets['Pensiun']->set_column($i,$i,$size_col[$val[$i]]+3);
          }
        }
