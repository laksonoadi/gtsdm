...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_pensiun/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetCountDataPensiun($awal, $akhir, $unit_kerja, $pangkat_golongan) {
      ...
      $result=$this->Open($sql, array($awal, $akhir, $satkerlevel, $satker));
      
  		return sizeof($result);
   }
   
   function GetDataPensiun($offset, $limit, $awal, $akhir, $unit_kerja, $pangkat_golongan) {
      $sql=$this->mSqlQueries['get_data_pensiun']; 
      $result=$this->Obj->GetSatkerDetail($unit_kerja);
      if($result){
          $satker = $result['satkerId'];
