...
$sql['get_data_user_by_id'] = "
   ...
   FROM
      ...
      LEFT JOIN gtfw_unit_application ua ON ua.unitappId = g.UnitappId
      LEFT JOIN gtfw_unit ON unitappUnitId = UnitId
      LEFT JOIN pub_pegawai_user ON peguserUserId=u.userId
   WHERE
      u.UserId = '%s'
";
//OK
...
//OK
$sql['get_data_pegawai_by_group'] = "
    SELECT 
      pegId AS id,
      concat(pegKodeResmi,' - ',pegNama) AS name
    FROM 
          pub_pegawai
          LEFT JOIN pub_pegawai_user ON peguserPegId=pegId
    WHERE pegId<>-1 AND 
          NOT (pegId IN (Select peguserPegId From pub_pegawai_user INNER JOIN gtfw_user_group ON userId=peguserUserId where groupId='%s'))
    GROUP BY pegId
    ORDER BY pegNama
";
...
$sql['do_add_user_pegawai'] = "
   INSERT INTO pub_pegawai_user
   SET
      peguserId=NULL,
      peguserUserId = %s,
      peguserPegId = %s
...
$sql['do_delete_user_by_id'] = 
   ...
   WHERE 
      UserId=%s";
$sql['do_delete_peg_user_by_id'] = 
   "DELETE from pub_pegawai_user
   WHERE 
      peguserUserId=%s";
//OK
$sql['do_delete_user_by_array_id'] = 
   ...
   WHERE 
      UserId IN ('%s')";
//OK
$sql['do_delete_peg_user_by_array_id'] = 
   "DELETE from pub_pegawai_user
   WHERE 
      peguserUserId IN ('%s')";
//OK
