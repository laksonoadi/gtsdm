...
class ViewDetailMutasi extends HtmlResponse
{
  ...
  function ProcessRequest()
  {
	...
	$return['link']['url_excel'] 	= Dispatcher::Instance()->GetUrl('laporan_bkd','LaporanBkdDetailIndv', 'view', 'xls').'&idBkd='.$id;
...
// exit;
  ...
  function ParseTemplate($data = NULL)
  {
    ...
    if($_GET['popupshow'] == 'oke'){
		$this->mrTemplate->AddVar('content', 'TOMBOLMAIN', '<a href="{URL_EXCEL}" style="cursor:pointer;" title="Export Excel" tabindex="3"><img src="images/button-excel.gif" alt="Export Excel">Export Excel</a>');
	} else {
		$this->mrTemplate->AddVar('content', 'TOMBOLMAIN', '<a href="{URL_BACK}" style="cursor:pointer;"><img src="images/button-back.gif" title="Kembali">Kembali</a>');
	}
         
    $link = $data['link'];
    $this->mrTemplate->AddVar('content', 'URL_BACK', $link['url_back']);
    $this->mrTemplate->AddVar('content', 'URL_EXCEL', $link['url_excel']);
