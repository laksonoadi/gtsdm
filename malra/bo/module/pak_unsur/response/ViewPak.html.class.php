...
class ViewPak extends HtmlResponse {
  ...
  function ProcessRequest() {
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('pak_unsur', 'Pak', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_ADD', Dispatcher::Instance()->GetUrl('pak_unsur', 'inputPak', 'view', 'html'));
      $this->mrTemplate->AddVar('content', 'TITLE', 'UNSUR KEGIATAN (ANGKA KREDIT DOSEN)');
      ...
      if (empty($data['pak'])) {
		 ...
		 $urlDelete = Dispatcher::Instance()->GetUrl('pak_unsur', 'deletePak', 'do', 'html');
		 $urlReturn = Dispatcher::Instance()->GetUrl('pak_unsur', 'Pak', 'view', 'html');
         ...
         foreach ($data['pak'] as $key => $value) {
            ...
            $urlAccept = 'pak_unsur|deletePak|do|html';
            $urlKembali = 'pak_unsur|Pak|view|html';
            $label = 'Unsur Kegiatan Angka Kredit Dosen';
            $dataName = $value['nama'];
            ...
            $value['URL_UPDATE'] = Dispatcher::Instance()->GetUrl('pak_unsur','inputPak', 'view', 'html').'&id='.Dispatcher::Instance()->Encrypt($value['id']);
