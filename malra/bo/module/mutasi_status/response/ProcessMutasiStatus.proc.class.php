...
class Process{
	...
	function AddData($nama_file){
		...
		if ($result){ //for update status aktif/tidak aktif
            ...
            $last = $this->Obj->GetLastMutasiStatus($this->POST['pegId']);
            if($last === TRUE || $last['id'] == $insert_id) {
                $params = array(
                    'pegIdStatHukum' => $insert_id,
                    'pegStatHukum' => $last['statrnama']
                );
