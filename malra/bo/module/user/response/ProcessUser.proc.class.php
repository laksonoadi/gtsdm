...
class ProcessUser {
   ...
   function __construct() {
      ...
      $this->pageView = Dispatcher::Instance()->GetUrl('user', 'user', 'view', 'html');
      $this->pageInput = Dispatcher::Instance()->GetUrl('user', 'inputUser', 'view', 'html');
      $this->pageInputPassword = Dispatcher::Instance()->GetUrl('user', 'changePassword', 'view', 'html');
   }

   function IsEmpty($formName, $label, $sub_modul) {
      if (isset($_POST['btnsimpan'])) {
			if(is_array($this->_POST[$formName])) {
				if(empty($this->_POST[$formName])){
					$this->SendAlert("Isian $label tidak boleh kosong.", $sub_modul);
					return true;
				}
			} elseif(trim($this->_POST[$formName]) == "") {
            $this->SendAlert("Isian $label tidak boleh kosong.", $sub_modul);
            return true;
         } else {
            return false;
   ...
   function SendAlert($alert, $sub_modul, $css='') {
      Messenger::Instance()->Send('user', $sub_modul, 'view', 'html', array($this->_POST, $alert, $css),Messenger::NextRequest);
   }
   ...
   function Add() {
      ...
      // $cek_unit = $this->IsEmpty('unit_kerja', 'Unit Aplikasi', 'inputUser');
      $cek_satker = $this->IsEmpty('satuan_kerja', 'Satuan Kerja', 'inputUser');
      $cek_group = $this->IsEmpty('group', 'Group', 'inputUser');
	   // $cek_satker = $this->IsEmpty('unit_satuan_kerja', 'Satuan Kerja', 'inputUser');
      
      $new_username = $_POST['username'];
      $cek = $this->userObj->GetCountDuplicateUsernameAdd($new_username);

      /* $jumlah = count($_POST["satuan_kerja"]);
      
      for($i=0; $i < $jumlah; $i++) 
      {  
          $nim=$_POST["satuan_kerja"][$i];
          $sat_ker .= $nim.',';
      } */
      
      if ($cek_user || $cek_pass || $cek_repass || $cek_unit || $cek_group || $cek_satker) {
         ...
         $addUserGroup = $this->userObj->DoAddUserGroup($last_insert_id, $_POST['group']);
		   ...
		   // $addUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($last_insert_id, $_POST['satuan_kerja']);
			
         //Add
			// $satker = array_unique($this->_POST['satuan_kerja']);
   //       $jumlah = count($satker);
      
   //       for($i=0; $i < $jumlah; $i++) 
   //       {
			// 	$addUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($last_insert_id, $satker[$i]);
			// 	if($addUserSatuanKerja === FALSE) break;
   //       }
         /* $satkerid = $this->userObj->GetSatkerById($_POST['unit_satuan_kerja']);
         if(!empty($satkerid)){
         $addUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($last_insert_id, $satkerid['0']['satkerId']);
         } */
         $addUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($last_insert_id, $_POST['satuan_kerja']);
			if ($addUserSatuanKerja === TRUE) {
				$this->SendAlert('Penambahan data Berhasil Dilakukan', 'user', $this->cssDone);
			}

         $addData = $addUser && $addUserDefGroup && $addUserGroup && $addUserSatuanKerja ;
/*var_dump($addUser);
...
var_dump($addUserGroup);exit;*/
   ...
   function Update() {
      ...
      if ($cek_user || $cek_unit || $cek_group || $cek_satker) {
         return $this->pageInput;
      } else {
         $new_username = $_POST['username'];
         ...
         $updateUserGroup = $this->userObj->DoUpdateUserGroup($_POST['group'], $this->decUsr);
		   ...
		   // $updateUserSatuanKerja = $this->userObj->DoUpdateUserSatuanKerja($_POST['satuan_kerja'], $this->decUsr);

         //delete
         $deleteUserSatuanKerja = $this->userObj->DoDeleteUserUnitById($this->decUsr);
   ...
   //       $jumlah = count($satker);
         ...
         $updateUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($this->decUsr, $satkerid['0']['satkerId']);
         } */
         $updateUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($this->decUsr, $_POST['satuan_kerja']);
			
         
    //      for($i=0; $i < $jumlah; $i++) 
    //      {
				// $updateUserSatuanKerja = $this->userObj->DoAddUserSatuanKerja($this->decUsr, $satker[$i]);
				// if($updateUserSatuanKerja === FALSE) break;
    //      }
			if ($updateUserSatuanKerja === TRUE) {
				$this->SendAlert('Pengubahan data Berhasil Dilakukan', 'user', $this->cssDone);
			}

         // $updateUserSatuanKerjaGrup = $this->userObj->DoUpdateUserSatuanKerjaGrup($sat_ker.'0', $this->decUsr);
         
         $updateData = $updateUser && $updateUserDefGroup && $updateUserGroup && $updateUserSatuanKerja && $deleteUserSatuanKerja;
