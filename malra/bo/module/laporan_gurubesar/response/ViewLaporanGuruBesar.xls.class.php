...
class ViewLaporanGuruBesar extends XlsResponse
{
   var $mWorksheets = array('GuruBesar');
   
   function GetFileName() {
      // name it whatever you want
      return 'Laporan_GuruBesar_'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      $this->Obj=new Laporan;
		  ...
		  $dataPegawai = $this->Obj->GetDataGuruBesar(0, $totalData, $this->unit_kerja);
  		$data=$dataPegawai;
  		$row=-1;
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$this->mWorksheets['GuruBesar']->merge_cells($row, 0, $row,$jumKolom-1);
		  ...
		  //Set Header
		  $header[0]=array('No.','NIP','Nama','Golongan','Jabatan','TMT','','Pendidikan','Kepakaran','Unit Kerja');
		  $header[1]=array('','','','','','Golongan','Jabatan','','','');
		  //Set Nama Variabel Yang akan ditulis
		  $val=array('no','nip','nama','gol','jabatan','gol_tmt','jabatan_tmt','pendidikan','kepakaran','unit_kerja');
