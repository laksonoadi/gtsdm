...
class Process
{
   ...
   function __construct($ret) {
      ...
      if($ret == "html"){
           if(isset($_POST['form_stat']) && $_POST['form_stat'] == 'analisa_jabatan'){
           $this->pageInput = Dispatcher::Instance()->GetUrl('analisa_jabatan','AnalisaJabatan','view','html');
           $this->pageView = Dispatcher::Instance()->GetUrl('analisa_jabatan','AnalisaJabatan','view','html');
           $this->pageBack = Dispatcher::Instance()->GetUrl('analisa_jabatan','AnalisaJabatan','view','html');          
           } else {
  ...
  function Check (){
    ...
    if ((trim($this->POST['jabs_ref']) == '' && empty($this->POST['old_jab']))){
  ...
  function AddData($nama_file){
    ...
    // Prioritas combobox, jika tidak kosong, maka kosongkan old_jab
    if(trim($this->POST['jabs_ref']) != '') {
        $jab = $this->POST['jabs_ref'];
        $old_jab = NULL;
    } else {
        $jab = 0;
        $old_jab = $this->POST['old_jab'];
    }
    ...
    if ($result){
	  ...
	  // }
       // Update last data
            ...
            $last = $this->Obj->GetLastMutasiJabStruk($this->POST['pegKode']);
            if($last === TRUE || $last['id'] == $insert_id) {
                $params = array(
                    'pegIdJabStruk' => $insert_id,
                    'pegJabStruk' => $last['jabStrukName']
                );
	...
	function InputData(){
      $check = $this->Check();
    if($_POST['form_stat'] == 'analisa_jabatan'){
      if($this->POST['dataStruk'] == $this->POST['jabs_ref']){
        $this->decId = $this->POST['dataKode'];
      }else {
        $this->decId = '';
      }      
    }

    // $check_pegawai_jobs = $this->Obj->GetCheckPegawaiNonJob($_POST['pegKode']);
    // // echo $check_pegawai_jobs;
    
    //     if($check_pegawai_jobs > 0 ){

    //           if($this->POST['form_stat'] == 'analisa_jabatan'){
    //            Messenger::Instance()->Send('analisa_jabatan','AnalisaJabatan','view','html', array($this->POST,$this->msgAddFail,$this->cssDone),Messenger::NextRequest);              
    //             // echo 'disini untuk anjab';
    //           } else{
    //             // echo 'disini untuk mutasi';
    //            Messenger::Instance()->Send('mutasi_jabatan_struktural','MutasiJabatanStruktural','view','html', array($this->POST,$this->msgAddFail,$this->cssDone),Messenger::NextRequest);              
    //           }
    //          // exit(); 
    //          return $this->pageView;
    //     }
        ...
        if($rs_add == true){
           ...
           if(($this->POST['status']==$this->activeStatus) and ($this->POST['kat']=="Academic")){
             $rs_add_2 = true; //$this->AddDataMutasi($this->POST['pegKode'],"input");
              ...
              if($this->POST['form_stat'] == 'analisa_jabatan'){
               Messenger::Instance()->Send('analisa_jabatan','AnalisaJabatan','view','html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);              
              } else{
               Messenger::Instance()->Send('mutasi_jabatan_struktural','MutasiJabatanStruktural','view','html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);              
              }
             return $this->pageView;
        ...
        if($rs_update == true){
           if (!empty($_FILES['file']['tmp_name'])){
             ...
             $rs_update_2 = true; //$this->AddDataMutasi($this->POST['pegKode'],"update");
              ...
              if($this->POST['form_stat'] == 'analisa_jabatan'){
               Messenger::Instance()->Send('analisa_jabatan','AnalisaJabatan','view','html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);              
              } else{
               Messenger::Instance()->Send('mutasi_jabatan_struktural','MutasiJabatanStruktural','view','html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);              
              }
