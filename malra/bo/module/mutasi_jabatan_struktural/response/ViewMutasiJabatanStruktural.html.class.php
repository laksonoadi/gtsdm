...
class ViewMutasiJabatanStruktural extends HtmlResponse
  ...
  function ProcessRequest() 
  {
    ...
    if(isset($_GET['id'])){
      ...
      $dataUnitKerja = $js->GetUnitKerjaPegawaiAktif($id);
      ...
      if(isset($_GET['dataId'])){
        ...
        if(!empty($result)){
          ...
          $return['input']['nip'] = $result['nip'];
			    ...
			    $return['input']['old_jab'] = $result['old_jab'];
          $return['input']['gaji_jab'] = $result['gaji_jab'];
        ...
        $return['input']['old_jab'] = '';
        $return['input']['gaji_jab'] = '';
        $return['input']['eselon'] = '';
    ...
    // print_r($return);
    // echo "</pre>";
    $arrjp = $js->GetComboJabatanStruktural($return['input']['struktural']);
       
       // if(!empty($dataUnitKerja)){ // activated for filter jabatan by unit or satker pegawai
       //  $arrjp = $js->GetComboJabatanStrukturalByUnit($dataUnitKerja['0']['id']);
       //   }else{
       //    $arrjp = $js->GetComboJabatanStruktural($return['input']['struktural']);
       //   }
    ...
    Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'jabs_ref', array('jabs_ref', $arrjp, $return['input']['struktural'], 'false', ' style="width:500px;" '), Messenger::CurrentRequest);
    ...
    $list_status=array(array('id'=>'Aktif','name'=>$active),array('id'=>'Tidak Aktif','name'=>$inactive));
	  ...
	  $list_eselon = $js->GetComboEselon();    
	
  Messenger::Instance()->SendToComponent('combobox', 'Combobox', 'view', 'html', 'eselon', array('eselon', $list_eselon, $return['input']['eselon'], 'false', 'id="eselon " style="width:200px;"  '), Messenger::CurrentRequest);
  ...
  function ParseTemplate($data = NULL)
  {
    ...
    if(!empty($data['input'])){
        ...
        $this->mrTemplate->AddVar('content', 'OLD_JAB_CHECKED', $data['input']['struktural'] === '0' ? 'checked="checked"' : '');
	  }
    ...
    if (empty($dataJabs)) {
      ...
      for ($i=0; $i<count($dataJabs); $i++) {
        ...
        $idPeg = Dispatcher::Instance()->Encrypt($dataPegawai[0]['id']);
        $idEnc = Dispatcher::Instance()->Encrypt($dataJabs[$i]['id']);
        $urlAccept = 'mutasi_jabatan_struktural|deleteMutasiJabatanStruktural|do|html-id-'.$idPeg;
        $urlKembali = 'mutasi_jabatan_struktural|MutasiJabatanStruktural|view|html-id-'.$idPeg;
        ...
        $dataJabs[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_jabatan_struktural','MutasiJabatanStruktural', 'view', 'html').'&id='.$idPeg.'&dataId='. $idEnc;
        $dataJabs[$i]['URL_DETAIL'] = Dispatcher::Instance()->GetUrl('mutasi_jabatan_struktural','detailMutasi', 'view', 'html').'&id='.$idPeg.'&dataId='. $idEnc;
        ...
        $path_jabstruk_template = GTFWConfiguration::GetValue('application', 'template_anjab');
        if (!empty($dataJabs[$i]['jabstruk_template']) && file_exists($path_jabstruk_template.$dataJabs[$i]['jabstruk_template'])) {
          $this->mrTemplate->addVar('data_item_sk', 'URL', Dispatcher::Instance()->GetUrl('mutasi_jabatan_struktural', 'rtfSkMutasiJabatanStruktural', 'view', 'html').'&id='.$idPeg.'&dataId='.$idEnc);
          $dataJabs[$i]['LINK_DOWNLOAD_SK'] = $data['link']['link_download'].$dataJabs[$i]['upload'];
        }
