...
class MutasiJabatanStruktural extends Database {
   ...
   function GetDataPegawaiByMutasiId($id, $pegId) {
      $result = $this->Open($this->mSqlQueries['get_data_pegawai_by_mutasi_id'], array($id, $pegId));
      return $result;
   }
...
//===============do======================//   
  ...
  function GetComboEselon(){
      $result = $this->Open($this->mSqlQueries['ref_combo_eselon'], array()); 
      return $result;    
  }

  function GetLastMutasiJabStruk($id) {
        $result = $this->Open($this->mSqlQueries['get_last_mutasi_jabstruk_pegawai'], array($id));
        if(is_array($result)) {
            if(count($result) > 0) {
