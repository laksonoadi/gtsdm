...
class ManejemenBerita extends Database {
   ...
   function ListBerita($offset,$limit) {
      ...
      //echo sprintf($this->mSqlQueries['list_berita'],$dateFormat,$offset,$limit);
		// var_dump(stripslashes($this->mSqlQueries['list_berita']), vsprintf(stripslashes($this->mSqlQueries['list_berita']), array($dateFormat,$_SESSION['username'],$offset,$limit)));
      // return $this->Open($this->mSqlQueries['list_berita'], array($dateFormat,$_SESSION['username'],$offset,$limit));
      return $this->Open($this->mSqlQueries['list_berita'], array($_SESSION['username'],$offset,$limit));
   }
   ...
   function AddBerita($nama,$artikel,$foto,$caption_foto,$status,$pengirim,$tanggal_berita){
      return $this->Execute($this->mSqlQueries['add_berita'],array($nama,$artikel,$foto,$caption_foto,$status,$pengirim,$tanggal_berita));
   }
   
   function UpdateBerita($nama,$artikel,$foto,$caption_foto,$status,$pengirim,$tanggal_berita,$id){
      return $this->Execute($this->mSqlQueries['update_berita'],array($nama,$artikel,$foto,$caption_foto,$status,$pengirim,$tanggal_berita,$id));
   }
