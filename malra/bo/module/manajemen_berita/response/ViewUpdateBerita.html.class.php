...
class ViewUpdateBerita extends HtmlResponse {
   ...
   function ProcessRequest() {
      ...
      Messenger::Instance()->SendToComponent('tanggal', 'Tanggal', 'view', 'html', 'tanggal_berita', array($data['berita'][0]['DATE_NEWS'], $tahun['start'], $tahun['end'], '', '', 'tanggal_berita'), Messenger::CurrentRequest);
   ...
   if($this->Pesan)
      ...
      if(!empty($data['berita'])){
         ...
         if(!empty($berita['FOTO'])){
            ...
            $this->mrTemplate->addVar('foto','FOTO_PATH',GTFWConfiguration::GetValue('application', 'file_berita').$berita['FOTO']);
         }
         $this->mrTemplate->addVar('content','CAPTION',$berita['CAPTION']);
         $this->mrTemplate->addVar('content','DATE_NEWS',$this->mBerita->IndonesianDate($berita['DATE_NEWS'],"yyyy-mm-dd","dd/mm/yyyy"));
   ...
   function DoUpdateBerita($fileName){
      ...
      return $this->mBerita->UpdateBerita($_POST['title'],$_POST['article'],$fileName,$_POST['caption'],$status,$sender,$date,$_POST['id']);
   }
}
?>