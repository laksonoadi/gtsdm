...
'module/agama/business/agama.class.php';

class ViewAgama extends HtmlResponse{

  function TemplateModule(){
    $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
    'module/agama/'.GTFWConfiguration::GetValue('application', 'template_address').'');
    $this->SetTemplateFile('view_agama.html');
  }
  ...
  function ProcessRequest(){
    ...
    if(isset($_GET['id'])){
      $result = $agama->GetDataById($id);
      if($result){
         $return['input']['nama'] = $result['agmNama'];
      }else{
         unset($_GET['id']);
      }
    }else{
    ...
    $totalData = $agama->GetCount($return['cari']);
    $url = Dispatcher::Instance()->GetUrl('agama','agama','view','html').'&cari='.$return['cari'];
    if (isset($_GET['id'])){ 
      $url .= '&id='.$id;
    }
    ...
    $return['link']['url_action'] = Dispatcher::Instance()->GetUrl('agama','inputAgama','do','html');
    if (isset($_GET['id'])){ 
      $return['link']['url_action'] .= '&id='.$id;
    }
    
    $return['link']['url_search'] = Dispatcher::Instance()->GetUrl('agama','agama','view','html');
    
    $return['link']['url_edit'] = Dispatcher::Instance()->GetUrl('agama','agama','view','html');
    if ($return['cari'] != ''){
    ...
    if ($lang=='eng'){
      ...
      $labeldel=Dispatcher::Instance()->Encrypt('Data Agama');
    }
    $return['lang']=$lang; 
    
    $return['link']['url_delete'] = Dispatcher::Instance()->GetUrl('confirm', 'confirmDelete', 'do', 'html').
    "&urlDelete=".Dispatcher::Instance()->Encrypt('agama|deleteAgama|do|html').
    "&urlReturn=".Dispatcher::Instance()->Encrypt('agama|agama|view|html').
    "&label=".$labeldel;
    $return['link']['url_delete_js'] = Dispatcher::Instance()->GetUrl('agama', 'deleteAgama', 'do', 'html');
    $return['dataSheet'] = $agama->GetData($startRec,$itemViewed,$return['cari']);//print_r($return);
  ...
  function ParseTemplate($data = NULL){
    ...
    if ($data['lang']=='eng'){
      ...
      $this->mrTemplate->AddVar('content', 'TITLE', 'REFERENSI AGAMA');
      $this->mrTemplate->AddVar('content', 'JUDUL_DATA', 'Data Agama');
    ...
    foreach ($data['dataSheet'] as $value){
      ...
      $data['url_edit'] = $link['url_edit'].'&id='.$data['agmId'];
      $data['url_delete'] = $link['url_delete'].
      "&id=".Dispatcher::Instance()->Encrypt($data['agmId']).
      "&dataName=".Dispatcher::Instance()->Encrypt($data['agmNama']);
