...
class ViewGroup extends HtmlResponse {
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('group_portal', 'group', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'GROUP_URL_ADD', Dispatcher::Instance()->GetUrl('group_portal', 'inputGroup', 'view', 'html') );
      ...
      if (empty($data['dataGroup'])) {
         ...
         for($i=1;$i<count($menuBaru)+1;$i++){            
            ...
            $menuBaru[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('group_portal', 'inputGroup', 'view', 'html') . '&grp=' . $idEnc;

            $idEnc = Dispatcher::Instance()->Encrypt($menuBaru[$i]['group_id']);
                        
            $urlAccept = 'group_portal|deleteGroup|do|html-cari-'.$cari;
            $urlReturn = 'group_portal|group|view|html-cari-'.$cari;
            $label = 'Group';
         ...
         /*for ($i=0; $i<$len; $i++) {
            ...
            $dataGroup[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('group_portal', 'inputGroup', 'view', 'html') . '&grp=' . $idEnc;
            ...
            //$dataGroup[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('group_portal', 'deleteGroup', 'do', 'html') . '&grp=' . $idEnc;
