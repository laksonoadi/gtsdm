...
WHERE
    (f.satkerId='%s' OR f.satkerLevel LIKE CONCAT('%s', '.%%')) AND jen.`jnspegrId` = '%s'
"; 
...
WHERE
    ...
    AND (f.satkerId='%s' OR f.satkerLevel LIKE CONCAT('%s', '.%%'))
    %s
GROUP BY a.pegId
";   
...
FROM
  ...
  LEFT JOIN sdm_status_pegawai k ON k.statpegId=a.pegId AND k.statpegAktif='Aktif'
  LEFT JOIN sdm_ref_status_pegawai l ON l.statrId = k.statpegStatrId */
  WHERE (f.satkerId='%s' OR f.satkerLevel LIKE CONCAT('%s', '.%%'))
