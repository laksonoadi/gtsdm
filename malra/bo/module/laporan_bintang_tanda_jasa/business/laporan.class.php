...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_bintang_tanda_jasa/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetComboTandaJasa(){
      return $this->Open($this->mSqlQueries['get_combo_tanda_jasa'], array());
   }
   
   /* function GetCountDataBintangTandaJasa($awal, $akhir, $unit_kerja, $tanda_jasa) {
      $sql=$this->mSqlQueries['get_data_kenaikanpangkat']; 
      ...
      if($tanda_jasa != "all"){
  		  $sql=str_replace('%tanda_jasa%'," AND tandaJasaId='".$tanda_jasa."'",$sql);
      }else{
        $sql=str_replace('%tanda_jasa%','',$sql);
      }
	...
	function GetCountDataBintangTandaJasa() {
		$query = $this->mSqlQueries['count_data_bintang_tanda_jasa'];
		$result = $this->Open($query, array());
		return $result[0]['total'];
	}
	
   function GetDataBintangTandaJasa($offset, $limit, $awal, $akhir, $unit_kerja, $tanda_jasa) {
      $sql=$this->mSqlQueries['get_data_bintang_tanda_jasa'];
