...
class Process
{
   ...
   function __construct($ret) {
      $this->Obj = new MutasiPangkatGolongan();
      ...
      if($ret == "html"){
         $this->pageInput = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html').'&id='.$this->profilId;
         $this->pageView = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html').'&id='.$this->profilId;
         $this->pageBack = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html').'&id='.$this->delId;
        }else{
         $this->pageInput = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html',true).'&id='.$this->profilId;
         $this->pageView = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html',true).'&id='.$this->profilId;
         $this->pageBack = Dispatcher::Instance()->GetUrl('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html').'&id='.$this->delId;
        }
  ...
  function AddData($nama_file){
    ...
    $tgl_bkn=$this->POST['tgl_bkn_year'].'-'.$this->POST['tgl_bkn_mon'].'-'.$this->POST['tgl_bkn_day'];
    $array=array(
				'pegKode'=>$this->POST['pegKode'],
				'jenis_pegawai'=>$this->POST['jenis_pegawai'],
				'golongan'=>$this->POST['golongan_ref'],
				'tmt'=>$tmt,
        'masa_thn' => $this->POST['masa_thn'],
        'masa_bln' => $this->POST['masa_bln'],
				'tgl_naik'=>$tgl_naik,
        ...
        'tgl_bkn'=>$tgl_bkn,
        'gaji_pangkat'=>$this->POST['gaji_pangkat'],
  ...
  //$this->dumper($array);exit();
    ...
    if ($result){
      ...
      // Update last data
            ...
            $last = $this->Obj->GetLastMutasiGolongan($this->POST['pegKode']);
            // print_r($last);
            // exit();
            if($last === TRUE || $last['id'] == $insert_id) {
                $params = array(
                    'pegIdPktGol' => $insert_id,
                    'pegPktGol' => $last['golongan']
                );
	...
	function InputData(){
      ...
      if ((isset($this->POST['btnsimpan'])) && empty($this->decId)) {
        ...
        if($rs_update == true){
           ...
           Messenger::Instance()->Send('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);
           return $this->pageView;
        }else{
           Messenger::Instance()->Send('mutasi_pangkat_golongan','MutasiPangkatGolongan','view','html', array($this->POST,$this->msgUpdateFail,$this->cssFail),Messenger::NextRequest);
           return $this->pageView;
        }
      }
      //return $urlRedirect;
   ...
   if($deleteData == true) {
			Messenger::Instance()->Send('mutasi_pangkat_golongan', 'MutasiPangkatGolongan', 'view', 'html', array($this->POST,$this->msgDeleteSuccess, $this->cssDone),Messenger::NextRequest);   
		}else{
			Messenger::Instance()->Send('mutasi_pangkat_golongan', 'MutasiPangkatGolongan', 'view', 'html', array($this->POST,$this->msgDeleteFail, $this->cssFail),Messenger::NextRequest);   
		}
