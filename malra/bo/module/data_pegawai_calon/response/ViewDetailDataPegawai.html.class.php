...
class ViewDetailDataPegawai extends HtmlResponse {
   ...
   function ProcessRequest() {
      ...
      $this->Data = array('id'=>$rs['id'],'nip'=>$rs['nip'],'kodegateaccess'=>$rs['kodegateaccess'],'kodeint'=>$rs['kodeint'],'kodelain'=>$rs['kodelain'],
		...
		'pnstmt'=>$rs['pnstmt'],'tglmasuk'=>$rs['tglmasuk'],'notaspen'=>$rs['notaspen'],'noaskes'=>$rs['noaskes'],'statusnpwp'=>$rs['statusnpwp'],
   ...
   function ParseTemplate($data = NULL) {      
      $this->mrTemplate->AddVar('content', 'URL_ACTION', Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'dataPegawai', 'view', 'html')); 
      ...
      $this->mrTemplate->AddVar('content', 'STATUSNPWP', $this->Data['statusnpwp']);
      ...
      if (!file_exists(GTFWConfiguration::GetValue( 'application', 'photo_save_path').$this->Data['foto']) | empty($this->Data['foto'])) { 
	  	...
	  	$this->mrTemplate->AddVar('content', 'DURASI', $this->Data['durasi']);
	  	$this->mrTemplate->AddVar('content', 'LEVEL', $this->Data['level']);
