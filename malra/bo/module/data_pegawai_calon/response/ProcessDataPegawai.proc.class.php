...
require_once GTFWConfiguration::GetValue( 'application', 'docroot').
   'module/data_pegawai_calon/business/data_pegawai.class.php';
...
class Process
{
   ...
   function __construct($ret) {
    ...
    if($ret == "html"){
      $this->pageInput = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'inputDataPegawai', 'view', 'html');
      $this->pageView = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'dataPegawai', 'view', 'html');
    }else{
      $this->pageInput = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'inputDataPegawai', 'view', 'html',true);
      $this->pageView = Dispatcher::Instance()->GetUrl('data_pegawai_calon', 'dataPegawai', 'view', 'html',true);
    }
  ...
  function Check (){
    ...
    if (/*(trim($this->POST['datpegNip']) == '') or */(trim($this->POST['datpegNama']) == '')/* or ($this->POST['pegId1'] == '') or ($this->POST['pegId2'] == '')*/){
    ...
    if (isset($error)){
      ...
      Messenger::Instance()->Send('data_pegawai_calon', 'inputDataPegawai', 'view', 'html', $msg, Messenger::NextRequest);
...
//    $c=$this->POST['datpegPnsTmt_year'].'-'.$this->POST['datpegPnsTmt_mon'].'-'.$this->POST['datpegPnsTmt_day'];
    ...
    $array=array('nip'=>$this->POST['datpegNip'],'kodeGateAccess'=>$this->POST['datpegKodeGateAccess'],'kodeInter'=>$this->POST['datpegKodeInter'],'kodeLain'=>$this->POST['datpegKodeLain'],
	  ...
	  'notaspen'=>$this->POST['datpegNoTaspen'],'noaskes'=>$this->POST['datpegNoAskes'],'statusnpwp'=>$this->POST['statusnpwp'],'nonpwp'=>$this->POST['datpegNoNpwp'],'usiapens'=>$usp,
	  'kodeabsen'=>$kda,'jnsidlain'=>$this->POST['datpegJenIdLain'],'jnspeg'=>$jnp,'statpeg'=>$stp,
      'satwilpeg'=>$s,'pegLevelId'=>$lvl,'pegStatusWargaNeg'=>$this->POST['pegStatusWargaNeg'],'foto'=>$nama_file,'userId'=>$this->user,'id'=>$this->POST['pegId']);
...
//            if($rs_add_2 == true){
               Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);
               return $this->pageView;
//            }else{
//               Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgAddFail,$this->cssFail),Messenger::NextRequest);
//               return $this->pageView;
//            }
         }else{
            Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgAddFail,$this->cssFail),Messenger::NextRequest);
...
/*      }else{
         Messenger::Instance()->Send('data_pegawai_calon', 'inputDataPegawai', 'view', 'html', array($this->POST,$this->msgAddFail2,$this->cssFail),Messenger::NextRequest);
         return $this->pageInput;
      }*/
...
//  	         if($rs_update_2 == true){
                Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);
                return $this->pageView;
/*             }else{
                Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgUpdateFail,$this->cssFail),Messenger::NextRequest);
                return $this->pageView;
             }*/
          }else{
             Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->POST,$this->msgUpdateFail,$this->cssFail),Messenger::NextRequest);
...
/*        }else{
   ...
   function Delete(){	
    ...
    if($deleteData == true) {
			Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->_POST,$this->msgDeleteSuccess, $this->cssDone),Messenger::NextRequest);   
		}else{
			Messenger::Instance()->Send('data_pegawai_calon', 'dataPegawai', 'view', 'html', array($this->_POST,$this->msgDeleteFail, $this->cssFail),Messenger::NextRequest);   
		}
