...
class ViewDataMertua extends HtmlResponse
{
   ...
   function ParseTemplate($data = NULL)
   {
    ...
    $this->mrTemplate->AddVar('content', 'URL_PGW', Dispatcher::Instance()->GetUrl('data_pegawai','detailDataPegawai', 'view', 'html').'&dataId='. $data['id']); 
      ...
      $this->mrTemplate->AddVar('content', 'PATH_AKTA', GTFWConfiguration::GetValue('application', 'akta_mertua_path'));
  ...
  //mulai bikin tombol delete
        ...
        for ($i=0; $i<count($dataMertua); $i++) {
          ...
          $dataMertua[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('data_mertua','dataMertua', 'view', 'html').'&dataId2='. $idEnc.'&dataId='. $idEnc2;
