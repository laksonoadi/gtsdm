...
class Process
{
  ...
  function AddDatmertua($file_akta_meninggal){
	...
	function UpdateDatmertua($file_akta_meninggal){ 
    $a=$this->POST['tgl_lahir_year'].'-'.$this->POST['tgl_lahir_mon'].'-'.$this->POST['tgl_lahir_day'];
    $c=$this->POST['tgl_meninggal_year'].'-'.$this->POST['tgl_meninggal_mon'].'-'.$this->POST['tgl_meninggal_day'];
	...
	function InputDatmertua(){
      ...
      if ((isset($this->POST['btnsimpan'])) && ($this->POST['op'] == 'add')) {
        // Nama file akta
        $path_akta = GTFWConfiguration::GetValue('application', 'akta_mertua_path');
        ...
        $rs_add = $this->AddDatmertua($file_akta_meninggal);
        $return = $this->pageView;
        $return .= "&dataId=".$this->POST['idPeg'];
        if($rs_add == true){
            // Commit file akta
        ...
        $detail = $this->Obj->GetDataMertuaDet($this->POST['idMertua']);
        $detail = $detail[0];
        ...
        $rs_update = $this->UpdateDatmertua($file_akta_meninggal);
        $return = $this->pageView;
        $return .= "&dataId=".$this->POST['idPeg'];
        if($rs_update == true){
            // Commit file akta
