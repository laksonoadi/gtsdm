...
class Process{
  ...
  function __construct() {
    $this->Obj = new HariLibur;
    $this->_POST = $_REQUEST->AsArray();
    $this->decId = $_GET['id']->Integer()->Raw();
    $this->user=Security::Authentication()->GetCurrentUser()->GetUserId();
    $this->encId = $this->decId;
    $this->pageView = Dispatcher::Instance()->GetUrl('hari_libur', 'hariLibur', 'view', 'html');
    $this->pageInput = Dispatcher::Instance()->GetUrl('hari_libur', 'hariLibur', 'view', 'html');
  ...
  function Check (){
    ...
    } elseif($this->POST['tgl_day'] == "0000" or $this->POST['tgl_mon'] == "00" or $this->POST['tgl_year'] == "00"){
      $error = $this->msgReqDataEmpty;
    }
    
    if (isset($error)){
      $msg = array($this->_POST, $error, $this->cssAlert);
      Messenger::Instance()->Send('hari_libur', 'hariLibur', 'view', 'html', $msg, Messenger::NextRequest);
  ...
  function Update(){
    ...
    $tgl=$this->_POST['tgl_year'].'-'.$this->_POST['tgl_mon'].'-'.$this->_POST['tgl_day'];

    $data=array('tgl'=>$tgl,'nama'=>$this->_POST['nama'],'keterangan'=>$this->_POST['keterangan'],'id'=>$this->decId);
  ...
  function Add(){
    ...
    $data=array('tgl'=>$tgl,'nama'=>$this->_POST['nama'],'keterangan'=>$this->_POST['keterangan']);//print_r($data);
