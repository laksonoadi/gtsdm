...
class StatusVerifikasi extends Database {

	protected $mSqlFile= 'module/ref_status_verifikasi/business/status_verifikasi.sql.php';
	...
	function Add($data) {	
		...
		$return = $this->Execute($this->mSqlQueries['do_add'], array($data['verstatName'],$data['verstatIsApproved'],$data['verstatIcon'],$userId));	  
		return $return;
	}  
  
	function Update($data) {
		$userId=Security::Authentication()->GetCurrentUser()->GetUserId();
		$return = $this->Execute($this->mSqlQueries['do_update'], array($data['verstatName'],$data['verstatIsApproved'],$data['verstatIcon'],$userId,$data['verstatId']));         		  
		return $return;
	}   
