...
class ViewPeriodeCuti extends HtmlResponse{
  ...
  function TemplateModule(){
    ...
    'module/ref_periode_cuti/'.GTFWConfiguration::GetValue('application', 'template_address').'');
  ...
  function ProcessRequest(){
    ...
    if(isset($_GET['id'])){
      ...
      if($result){
         $return['input']['total'] = $result['periodecutiTotal'];
         $return['input']['awal'] = $result['periodecutiAwal'];
         $return['input']['akhir'] = $result['periodecutiAkhir'];
         $return['input']['perId'] = $result['periodecutiId'];
         $return['input']['status'] = $result['periodecutiStatus'];
      }else{
         unset($_GET['id']);
      }
    }else{
    ...
    $url = Dispatcher::Instance()->GetUrl('ref_periode_cuti','periodeCuti','view','html').'&cari='.$return['cari'];
    if (isset($_GET['id'])){ 
	...
	//print_r($searchAwal);die;
    if (isset($getSearchAwal)){
    ...
    $return['link']['url_action'] = Dispatcher::Instance()->GetUrl('ref_periode_cuti','inputPeriodeCuti','do','html');
    if (isset($_GET['id'])){ 
      $return['link']['url_action'] .= '&id='.$id .'&pegId='.$pegId; 
    }
    
    $return['link']['url_search'] = Dispatcher::Instance()->GetUrl('ref_periode_cuti','periodeCuti','view','html').'&cutiperAwalSearch='.$return['cutiperAwalSearch'].'&cutiperAkhirSearch='.$return['cutiperAkhirSearch'];
    
    //print_r($return['link']['url_search']);die;
    
    $return['link']['url_edit'] = Dispatcher::Instance()->GetUrl('ref_periode_cuti','periodeCuti','view','html');
    ...
    "&urlDelete=".Dispatcher::Instance()->Encrypt('ref_periode_cuti|deletePeriodeCuti|do|html').
    "&urlReturn=".Dispatcher::Instance()->Encrypt('ref_periode_cuti|periodeCuti|view|html').
    "&label=".$labeldel;
    $return['link']['url_delete_js'] = Dispatcher::Instance()->GetUrl('ref_periode_cuti', 'deletePeriodeCuti', 'do', 'html');
    ...
    $return['dataSheet'] = $periodeCuti->GetData($startRec,$itemViewed);//print_r($return);   
  ...
  function ParseTemplate($data = NULL){
    ...
    if ($data['lang']=='eng'){
      ...
      $this->mrTemplate->AddVar('content', 'TITLE', 'REFERENSI PERIODE CUTI');
	...
	$this->mrTemplate->AddVar('content', 'PERID', $data['input']['perId']);
    ...
    foreach ($data['dataSheet'] as $value){
      ...
      $data['url_edit'] = $link['url_edit'].'&id='.$data['periodecutiId'];
      $data['url_delete'] = $link['url_delete'].
      "&id=".Dispatcher::Instance()->Encrypt($data['periodecutiId']);
      $data['url_delete_js'] = $link['url_delete_js'];
      $this->mrTemplate->AddVars('data_item', $data, '');
      $data['periodecutiAwal'] = $this->periode2string($data['periodecutiAwal']);
      $this->mrTemplate->AddVar('data_item', 'CUTIPERAWAL', $data['periodecutiAwal']);
      $data['periodecutiAkhir'] = $this->periode2string($data['periodecutiAkhir']);
      $this->mrTemplate->AddVar('data_item', 'CUTIPERAKHIR', $data['periodecutiAkhir']);
	  $this->mrTemplate->AddVar('data_item', 'CUTIPERTOTAL', $data['periodecutiTotal']);
	  $this->mrTemplate->AddVar('data_item', 'CUTIPERID', $data['periodecutiId']);
      if($data['cutiperStatus'] == 'Active'){
        $this->mrTemplate->AddVar('data_item', 'CUTIPERSTATUS', '<b>'.$data['periodecutiStatus'].'</b>');
      } else {
        $this->mrTemplate->AddVar('data_item', 'CUTIPERSTATUS', '<i>'.$data['periodecutiStatus'].'</i>');
      }
