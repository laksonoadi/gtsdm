...
class Laporan extends Database {
   ...
   protected $mSqlFile= 'module/laporan_daftar_pegawai_berdasarkan_status/business/laporan.sql.php';
...
//==GET== 
   ...
   function GetComboStatusPegawai(){
      return $this->Open($this->mSqlQueries['get_combo_status_pegawai'],array());
   }
   
   function GetCountDataPensiun($unit_kerja, $status_pegawai) {
      ...
      if($status_pegawai != "all"){
  		  $sql=str_replace('%status_pegawai%'," AND statrId='".$status_pegawai."'",$sql);
      }else{
        $sql=str_replace('%status_pegawai%','',$sql);
      }
   ...
   function GetDataPensiun($offset, $limit,$unit_kerja, $status_pegawai) {
      $sql=$this->mSqlQueries['get_data_pensiun']; 
