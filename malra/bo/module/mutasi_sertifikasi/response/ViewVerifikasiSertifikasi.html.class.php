...
class ViewVerifikasiSertifikasi extends HtmlResponse{
	...
	function ParseTemplate($data = NULL){
		...
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('mutasi_sertifikasi', 'verifikasiSertifikasi', 'view', 'html').'&srtfkId='.$_GET['srtfkId']);
		...
		if(empty($cariData)) {
			...
			for ($i=0; $i<sizeof($cariData); $i++) {
				...
				$cariData[$i]['srtfkdetVerifikasi_status']=$cariData[$i]['srtfkdetIsVerifikasi']==1?'Sudah':'';
