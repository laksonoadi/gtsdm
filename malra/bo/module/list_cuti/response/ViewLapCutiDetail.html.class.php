...
require_once GTFWConfiguration::GetValue( 'application', 'docroot').
   'module/list_cuti/business/cuti.class.php';
   
class ViewLapCutiDetail extends HtmlResponse
{
   function TemplateModule()
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').
         'module/list_cuti/'.GTFWConfiguration::GetValue('application', 'template_address').'');
      $this->SetTemplateFile('view_lap_cuti_detail.html');
   }
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $buttonlang=GTFWConfiguration::GetValue('application', 'button_lang');
       if ($buttonlang=='eng'){
         $this->mrTemplate->AddVar('content', 'TITLE', 'LEAVE REPORT');
         $this->mrTemplate->AddVar('content', 'LABEL_ACTION', isset($_GET['dataId2']) ? 'Update' : 'Add');
       }else{
         $this->mrTemplate->AddVar('content', 'TITLE', 'LAPORAN CUTI');
      ...
      $this->mrTemplate->AddVar('content', 'URL_BACK', Dispatcher::Instance()->GetUrl('list_cuti', 'lapDataCuti', 'view', 'html') );
      ...
      */
      ...
      if((empty($dat2['nmp'])) or (empty($dat3['nmp']))){
        $this->mrTemplate->AddVar('data', 'EMPTY', 'YES');
      }else{
        $this->mrTemplate->AddVar('data', 'EMPTY', 'NO');
        $this->mrTemplate->AddVars('data_item', $dat2, '2_');
        $this->mrTemplate->AddVars('data_item', $dat3, '3_');
      }
