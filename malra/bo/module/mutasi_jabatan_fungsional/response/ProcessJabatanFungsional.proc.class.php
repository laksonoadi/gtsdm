...
class Process
{
  ...
  function Check (){
    ...
    if ((trim($this->POST['ref_jab']) == '' && !isset($this->POST['old_fjab_check']))){
  ...
  function AddData($nama_file){
    ...
    // Prioritas combobox, jika tidak kosong, maka kosongkan old_fjab
    if(trim($this->POST['ref_jab']) != '') {
        $jab = $this->POST['ref_jab'];
        $old_fjab = NULL;
    } else {
        $jab = 0;
        $old_fjab = $this->POST['old_fjab'];
    }
    
    $array=array(
				'pegKode'=>$this->POST['pegKode'],
        'ref_jenis_jab'=>$this->POST['ref_jenis_jab'],
				'ref_jab'=>$jab,
    ...
    if ($result){
	  ...
	  // }
            ...
            $last = $this->Obj->GetLastMutasiJabFung($this->POST['pegKode']);
            if($last === TRUE || $last['id'] == $insert_id) {
                $params = array(
                    'pegIdJabFung' => $insert_id,
                    'pegJabFung' => $last['jabFungName']
                );
