...
//get data profil   
   ...
   function GetJabatan ($offset, $limit, $pegId) { 
     $arg='AND a.jbtnPegKode= \'%s\' ORDER BY a.jbtnTglMulai DESC, a.jbtnId DESC ';
	 $params=array($pegId);
...
//===============do======================// 
  ...
  function GetLastMutasiJabFung($id) {
        $result = $this->Open($this->mSqlQueries['get_last_mutasi_jabfung_pegawai'], array($id));
        if(is_array($result)) {
            if(count($result) > 0) {
