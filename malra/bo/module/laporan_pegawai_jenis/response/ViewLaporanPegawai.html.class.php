...
class ViewLaporanPegawai extends HtmlResponse
{
   ...
   {
      $this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/laporan_pegawai_jenis/'.GTFWConfiguration::GetValue('application', 'template_address').'');
   ...
   function ParseTemplate($data = NULL)
   {
      ...
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('laporan_pegawai_jenis', 'laporanPegawai', 'view', 'html') );
      $this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('laporan_pegawai_jenis', 'laporanPegawai', 'view', 'xls')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
      $this->mrTemplate->AddVar('content', 'URL_RTF', Dispatcher::Instance()->GetUrl('laporan_pegawai_jenis', 'rtfLaporanPegawai', 'view', 'html')
        .'&jabatan_fungsional=' . Dispatcher::Instance()->Encrypt($this->jabatan_fungsional)
        .'&jenis_pegawai=' . Dispatcher::Instance()->Encrypt($this->jenis_pegawai)
        .'&cari=' . Dispatcher::Instance()->Encrypt(1));
        ...
        if (empty($this->ComboUnitKerja)) {
    			...
    			$this->mrTemplate->AddVar('content', 'GOLONGAN_COLSPAN', $totalKolomJenisPegawai*2);
    			
    			//Header Pendidikan
    			for ($i=0; $i<$totalKolomJenisPegawai; $i++){
    			   $this->mrTemplate->AddVar('kolom_jenis','GOLONGAN',$this->ComboJenisPegawai[$i]['name']);
    			   $this->mrTemplate->parseTemplate('kolom_jenis', 'a');
          }
