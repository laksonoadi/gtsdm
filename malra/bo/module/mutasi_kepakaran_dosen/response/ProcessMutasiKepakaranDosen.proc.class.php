...
class Process
{
   ...
   function __construct($ret) {
		...
		if($ret == "html"){	
			...
			$this->pageInput = Dispatcher::Instance()->GetUrl('mutasi_kepakaran_dosen','MutasiKepakaranDosen','view','html',true).'&id='.$this->profilId;
			$this->pageView = Dispatcher::Instance()->GetUrl('mutasi_kepakaran_dosen','MutasiKepakaranDosen','view','html',true).'&id='.$this->profilId;
			$this->pageBack = Dispatcher::Instance()->GetUrl('mutasi_kepakaran_dosen','MutasiKepakaranDosen','view','html').'&id='.$this->delId;
			$this->lang = GTFWConfiguration::GetValue('application', 'button_lang');
		}
		if ($this->lang=='eng'){
			...
			$this->error='Lecturer expertise data has not been added!';
      	}else{
