...
class ViewMutasiKepakaranDosen extends HtmlResponse
   {
	  ...
	  if($data['lang']=='eng') {
		...
		$this->mrTemplate->AddVar('content', 'TITLE', "RIWAYAT BIDANG KEPAKARAN DOSEN");
      ...
      if (empty($dataBidang)) {
			...
			for ($i=0; $i<count($dataBidang); $i++) {
				...
				$dataBidang[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_kepakaran_dosen','MutasiKepakaranDosen', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
				...
				if (!empty($dataBidang[$i]['upload'])){
					...
					$dataBidang[$i]['VIEW_DOWNLOAD'] = 'none';
				}
