...
class ViewHistoryDataSertifikasi extends HtmlResponse {
	...
	function ParseTemplate($data = NULL) {
		$this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'historyDataSertifikasi', 'view', 'html') . "&dataId=" . $this->encDataId);
		$this->mrTemplate->AddVar('content', 'URL_TAMBAH_SERTIFIKASI', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'dataSertifikasi', 'view', 'html'));
		$this->mrTemplate->AddVar('content', 'URL_EXCEL', Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'xls').'&source=usulan&srtfkId=ALL&srtfkdetHasilAkhir=ALL');
		if($this->Pesan) {
		...
		if (empty($data['dataHistory'])) {
			...
			for ($i=0; $i<sizeof($dataHistory); $i++) {
				...
				$dataHistory[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'deleteDataSertifikasi', 'view', 'html') . '&srtfkId=' . $idEnc;
				$dataHistory[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'detailSertifikasi', 'view', 'html') . '&srtfkId=' . $idEnc;
				$dataHistory[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'dataSertifikasi', 'view', 'html') . '&srtfkId=' . $idEnc;
				$dataHistory[$i]['url_verifikasi'] = Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'verifikasiSertifikasi', 'view', 'html') . '&srtfkId=' . $idEnc;
				$dataHistory[$i]['url_penilaian'] = Dispatcher::Instance()->GetUrl('sertifikasi_usulan', 'penilaianSertifikasi', 'view', 'html') . '&srtfkId=' . $idEnc;
