...
class Process
{
   ...
   function __construct() {
    $this->Obj = new OrangTua();
	  $this->user=Security::Authentication()->GetCurrentUser()->GetUserId();
	  $this->pageView = Dispatcher::Instance()->GetUrl('data_orang_tua', 'dataOrangTua', 'view', 'html');
  ...
  function Check (){
    ...
    if (isset($error)){
      ...
      Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', $msg, Messenger::NextRequest);
      
      $return = $this->pageView;
      /*if (isset($_GET['dataId2'])){ 
      $return .= "&dataId2=".$this->decId;
      }*/
      if (($this->POST['op']=="edit") and (isset($this->POST['idOrtu']))){ 
      $return .= "&dataId2=".$this->POST['idOrtu'];
      }
  ...
  function AddDatortu($file_akta_meninggal){
	...
	function UpdateDatortu($file_akta_meninggal){ 
    $a=$this->POST['tgl_lahir_year'].'-'.$this->POST['tgl_lahir_mon'].'-'.$this->POST['tgl_lahir_day'];
    $c=$this->POST['tgl_meninggal_year'].'-'.$this->POST['tgl_meninggal_mon'].'-'.$this->POST['tgl_meninggal_day'];
	...
	function InputDatortu(){
      $check = $this->Check();
      if ($check !== true) return $check;
      if ((isset($this->POST['btnsimpan'])) && ($this->POST['op'] == 'add')) {
        // Nama file akta
        $path_akta = GTFWConfiguration::GetValue('application', 'akta_ortu_path');
        ...
        $rs_add = $this->AddDatortu($file_akta_meninggal);
        $return = $this->pageView;
        $return .= "&dataId=".$this->POST['idPeg'];
        if($rs_add == true){
            // Commit file akta
           ...
           Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);
           return $return;
        }else{
           Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgAddFail,$this->cssFail),Messenger::NextRequest);
           return $return;
        }
        
      }else if ((isset($this->POST['btnsimpan'])) && ($this->POST['op'] == 'edit')) {
        $detail = $this->Obj->GetDataOrtuDet($this->POST['idOrtu']);
        $detail = $detail[0];
        ...
        $rs_update = $this->UpdateDatortu($file_akta_meninggal);
        $return = $this->pageView;
        $return .= "&dataId=".$this->POST['idPeg'];
        if($rs_update == true){
            // Commit file akta
           ...
           Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);
           return $return;
        }else{
           Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgUpdateFail,$this->cssFail),Messenger::NextRequest);
           return $return;
        }
      }
   }
   ...
   function Delete(){
    ...
    if($deleteData == true) {
			Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgDeleteSuccess, $this->cssDone),Messenger::NextRequest);   
		}else{
			Messenger::Instance()->Send('data_orang_tua', 'dataOrangTua', 'view', 'html', array($this->POST,$this->msgDeleteFail, $this->cssFail),Messenger::NextRequest);   
		}
    $return = $this->pageView;
