...
class ViewLaporanPegawai extends XlsResponse
{
   ...
   function GetFileName() {
      ...
      return 'Laporan_Pertumbuhan_Pegawai'.date('Ymd').'.xls';
   }
   ...
   function ProcessRequest()
   {
      ...
      $this->fColData = $this->mrWorkbook->add_format();
    	...
    	$jumKolom=$totalKolomTahun+4;
  		$row++;
		  $this->mWorksheets['Pegawai']->write($row, 0, 'LAPORAN PERTUMBUHAN PEGAWAI', $this->fH1);
    	$this->mWorksheets['Pegawai']->merge_cells($row, 0, $row,$jumKolom-1);
    			  
		  $row++;
		  $this->mWorksheets['Pegawai']->write($row, 0, 'Jenis Pegawai  : '.$this->label_jenis_pegawai , $this->fH2);
		  $this->mWorksheets['Pegawai']->merge_cells($row, 0, $row,$jumKolom-1);
		  ...
		  $row++;
		  $this->mWorksheets['Pegawai']->write($row, 0, 'Tahun  : '.$this->tahun1.' s/d '.$this->tahun2 , $this->fH2);
		  $this->mWorksheets['Pegawai']->merge_cells($row, 0, $row,$jumKolom-1);
		  ...
		  for ($i=0; $i<$totalKolomTahun; $i++){
		    if ($i==0){
		      array_push($header[0],'Tahun Masuk');
		      array_push($header[0],'');
        }else{
          array_push($header[0],'');
		      array_push($header[0],'');
      ...
      array_push($header[0],'');
		  ...
		  $header[1]=array('','');
		  for ($i=0; $i<$totalKolomTahun; $i++){
		    array_push($header[1],$this->Tahun[$i]['name']);
		    array_push($header[1],'');
      }
      ...
      $header[3]=array(' ','Jenis Kelamin');
		  ...
		  for ($i=0; $i<$totalKolomTahun; $i++){
		    array_push($val,$this->Tahun[$i]['id'].'_l');
		    array_push($val,$this->Tahun[$i]['id'].'_p');
      }
      ...
      if (sizeof($this->ComboUnitKerja)<=0) {
          $row++;
            ...
            $this->mWorksheets['Pegawai']->write($row, $i, '', $this->fH3);
              ...
              $dataUnit[$i]['unit_kerja']=str_replace('&nbsp;','',$this->ComboUnitKerja[$i]['name']);
                 ...
                 $dataUnit[$i][$this->Tahun[$ii]['id'].'_l']=$tempDataL;
                 $dataUnit[$i][$this->Tahun[$ii]['id'].'_p']=$tempDataP;
              }
