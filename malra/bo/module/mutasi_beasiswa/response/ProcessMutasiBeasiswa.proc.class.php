...
class Process
{
   ...
   function __construct($ret) {
		$this->Obj = new MutasiBeasiswa();
		$this->user=Security::Authentication()->GetCurrentUser()->GetUserId();
		$this->decId = $_POST['dataKode']->Integer()->Raw();
		$this->profilId = $_POST['pegKode']->Integer()->Raw();
		$this->delId = $_GET['id']->Integer()->Raw();
		if($ret == "html"){
			$this->pageInput = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html').'&id='.$this->profilId;
			$this->pageView = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html').'&id='.$this->profilId;
			$this->pageBack = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html').'&id='.$this->delId;
		}else{
			$this->pageInput = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html',true).'&id='.$this->profilId;
			$this->pageView = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html',true).'&id='.$this->profilId;
			$this->pageBack = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa','view','html').'&id='.$this->delId;
		}
  ...
  function Check (){
    ...
    if (isset($this->error)){
      ...
      Messenger::Instance()->Send('mutasi_beasiswa','MutasiBeasiswa','view','html', $msg, Messenger::NextRequest);
  ...
  function AddData($nama_file){
	  ...
	  if (!empty($this->POST['tahun'])) {
			$tahun = $this->POST['tahun'];
		}else{
			$tahun = '0';
		}
	   if (!empty($this->POST['bulan'])) {
      ...
      $array=array(
                  ...
                  'tahun_diterima'=>$this->POST['tahun_diterima'],
      ...
      $result = $this->Obj->Add($array);
         if ($result){
			return $result;
            #Messenger::Instance()->Send('mutasi_beasiswa','MutasiMengajarDiluar','view','html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);
         }else{
			return false;
	...
	function InputData(){
      ...
      if ((isset($this->POST['btnsimpan'])) && empty($this->decId)) {
        ...
        if($rs_add == true){
           ...
           Messenger::Instance()->Send('mutasi_beasiswa','MutasiBeasiswa','view','html', array($this->POST,$this->msgAddSuccess,$this->cssDone),Messenger::NextRequest);
           return $this->pageView;
        }else{
           Messenger::Instance()->Send('mutasi_beasiswa','MutasiBeasiswa','view','html', array($this->POST,$this->msgAddFail,$this->cssFail),Messenger::NextRequest);
           return $this->pageView;
        }
        ...
        if($rs_update == true){
           ...
           Messenger::Instance()->Send('mutasi_beasiswa','MutasiBeasiswa','view','html', array($this->POST,$this->msgUpdateSuccess,$this->cssDone),Messenger::NextRequest);
           return $this->pageView;
        }else{
           Messenger::Instance()->Send('mutasi_beasiswa','MutasiBeasiswa','view','html', array($this->POST,$this->msgUpdateFail,$this->cssFail),Messenger::NextRequest);
           return $this->pageView;
        }
      }
      //return $urlRedirect;
   ...
   if($deleteData == true) {
			Messenger::Instance()->Send('mutasi_beasiswa', 'MutasiBeasiswa', 'view', 'html', array($this->POST,$this->msgDeleteSuccess, $this->cssDone),Messenger::NextRequest);   
		}else{
			Messenger::Instance()->Send('mutasi_beasiswa', 'MutasiBeasiswa', 'view', 'html', array($this->POST,$this->msgDeleteFail, $this->cssFail),Messenger::NextRequest);   
		}
