...
class ViewMutasiBeasiswa extends HtmlResponse
   {
      ...
      //print_r($dataMgj);
	  if($data['lang']=='eng') {
		$this->mrTemplate->AddVar('content', 'TITLE', 'SCHOLARSHIP HISTORY');
		...
		$label = "Management of Scholarship Data";
		$label_data = 'Scholarship History';
      } else {
		...
		$this->mrTemplate->AddVar('content', 'TYPE', isset($_GET['dataId']) ? 'submit' : 'reset');
		$label = "Manajemen Data Mutasi Beasiswa";
		$label_data = 'Riwayat Beasiswa';
	  }
      ...
      $urlKembali = 'mutasi_beasiswa|MutasiBeasiswa|view|html-id-'.$dataPegawai[0]['id'];
         ...
         $dataMgj[$i]['URL_EDIT'] = Dispatcher::Instance()->GetUrl('mutasi_beasiswa','MutasiBeasiswa', 'view', 'html').'&id='.$dataPegawai[0]['id'].'&dataId='. $idEnc;
