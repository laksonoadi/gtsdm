...
$sql['get_data_bazetting']="
SELECT
	nama,nip,jenis_kelamin,jenis_pegawai,golongan,golongan_tmt,
	%select_jabatan%, jabatan_tmt, jabstrukrTingkat as jabatan_tingkat,
	(CASE
	 WHEN jbtnJabstrukrId IS NOT NULL THEN 'struktural'
	 WHEN jbtnJabfungrId IS NOT NULL THEN 'fungsional'
	 ELSE 'nothing'
	END) as jns_jabatan,
...
FROM
	(SELECT
		...
		jabstrukrNama,
		jabfungrNama,
		struktur.jbtnTglMulai AS jabatan_tmt,
		jabstrukrTingkat,
		jbtnEselon,
	...
	FROM
		...
		*/
		...
		LEFT JOIN sdm_jabatan_struktural struktur ON struktur.jbtnPegKode=pegId AND struktur.jbtnStatus='Aktif'
		LEFT JOIN sdm_ref_jabatan_struktural ON jbtnJabstrukrId=jabstrukrId
	
		LEFT JOIN sdm_jabatan_fungsional fung ON fung.jbtnPegKode=pegId AND fung.jbtnStatus='Aktif'
	...
	WHERE
		...
		%jenis_jabatan%
  	%unit_kerja%
	ORDER BY (jbtnJabstrukrId IS NULL) ASC, jabstrukrTingkat ASC, (jbtnJabfungrId IS NULL) ASC, pktgolrTingkat DESC, pktgolrUrut DESC, pelThnIjazah DESC, pddkThnLulus DESC ) as temp
GROUP BY id
ORDER BY (jbtnJabstrukrId IS NULL) ASC, jabstrukrTingkat ASC, (jbtnJabfungrId IS NULL) ASC, pktgolrTingkat DESC, pktgolrUrut DESC, CAST(pegPnsTmt AS DATE) ASC
%limit%
";
