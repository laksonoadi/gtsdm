...
class ProcessGroup {
   ...
   function __construct () {
      ...
      $this->pathPanduan = GTFWConfiguration::GetValue('application', 'docroot');
      $this->pathPanduan = rtrim($this->pathPanduan, '/') . '/' . GTFWConfiguration::GetValue('application', 'panduan_group_path');
   }
   ...
   function Add() {
      ...
      if ($cek_group) {
         ...
         if ($_POST['groupname']!="") {
            ...
            // Upload file
            $upload_panduan = false;
            $nama_file = NULL;
            if(!empty($_FILES['guidefile']['tmp_name'])) {
                $upload_panduan = true;
                $nama_file = date('ymdHis') .'-'. str_replace(' ', '_', trim($_FILES['guidefile']['name']));
            }
            if($upload_panduan) {
                // Upload yang sebenarnya disini
                move_uploaded_file($_FILES['guidefile']['tmp_name'], $this->pathPanduan.$nama_file);
            }
            
            $this->groupObj->StartTrans();
         
            $addGroup = $this->groupObj->DoAddGroup($this->applicationId,$_POST['groupname'], $_POST['description'], $_POST['unit_kerja'], $nama_file);         
         ...
         if ($addMenu == true) {
            $this->SendAlert('Penambahan data Berhasil Dilakukan', 'group', $this->cssDone);
               ...
               // Gagal; delete file yang telah diupload
               if($upload_panduan) {
                   unlink($this->pathPanduan.$nama_file);
               }
   ...
   function Update() {
      ...
      $dataGroup = $this->groupObj->GetDataGroupById($idDec);
      $detail = $dataGroup[0];
      $cek_group = $this->IsEmpty('groupname', 'Nama Group', 'inputGroup');
      // $cek_unit = $this->IsEmpty('unit_kerja', 'Unit Kerja', 'inputGroup');
      if ($cek_group ) {
         ...
         if ($_POST['groupname']!="") {
            ...
            if(!empty($_FILES['guidefile']['tmp_name'])) {
                $upload_panduan = true;
                $nama_file = date('ymdHis') .'-'. str_replace(' ', '_', trim($_FILES['guidefile']['name']));
            } elseif(isset($_POST['guidefile_delete'])) {
                $nama_file = NULL;
            } else {
                $nama_file = $detail['guidefile'];
            }
            if($upload_panduan) {
                // Upload yang sebenarnya disini
                move_uploaded_file($_FILES['guidefile']['tmp_name'], $this->pathPanduan.$nama_file);
            ...
            $this->groupObj->StartTrans(); 
            $updateGroup = $this->groupObj->DoUpdateGroup($this->applicationId, $_POST['groupname'], $_POST['description'], $_POST['unit_kerja'], $nama_file, $idDec);
            $updateMenu = false;
         ...
         if ($updateMenu === true) {
            // Berhasil; delete file yang lama
            if($upload_panduan || isset($_POST['guidefile_delete'])) {
                unlink($this->pathPanduan.$detail['guidefile']);
            }
