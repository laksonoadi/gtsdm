...
$sql['get_data_group_by_id_detail'] = 
   "SELECT 
      GroupId AS id,
      GroupName AS groupname,
      g.Description AS description,
...
$sql['get_data_guide_group_by_id_detail'] = 
   "SELECT 
      GroupId AS id,
      GroupName AS groupname,
      g.Description AS description,
   ...
   WHERE
      g.GroupId like '%s' 
      AND (g.GuideFile IS NOT NULL AND g.GuideFile <> '')
   /*AND 
      GroupId != '2'*/
   ORDER BY 
      GroupName";

$sql['get_data_group_admin_user'] = 
   "SELECT 
      GroupId AS id,
      GroupName AS groupname,
      g.Description AS description,
   ...
   FROM 
      ...
      LEFT JOIN gtfw_unit gu ON unitappUnitId = UnitId
   WHERE
      GroupName like '%s' AND GroupId NOT IN(59)
   /*AND 
      GroupId != '2'*/
   ORDER BY 
      GroupName";
...
$sql['get_data_group_with_privilege_by_id_detail'] = "
SELECT 
      g.GroupId AS group_id, 
      g.GroupName AS group_name, 
      g.Description AS group_description, 
...
$sql['get_data_group_with_privilege_admin_user'] = "
SELECT 
      g.GroupId AS group_id, 
      g.GroupName AS group_name, 
      g.Description AS group_description, 
...
$sql['get_data_group_by_id']= 
   "SELECT 	
      ...
      GROUP_CONCAT(`MenuName` ORDER BY `MenuName` SEPARATOR '|') AS `menu_name`,
      GROUP_CONCAT(`ParentMenuId` ORDER BY `MenuName` SEPARATOR '|') AS `parent_menu`,
      GROUP_CONCAT(`MenuMenuId` ORDER BY `MenuName` SEPARATOR '|') AS `menu_menu_id`
   FROM 
    	gtfw_group g
    	JOIN gtfw_unit_application ut ON ut.unitappId=g.UnitappId
...
$sql['do_add_group'] = 
   "INSERT INTO gtfw_group 
      (GroupName, Description, UnitappId, GuideFile)
   VALUES
      ('%s','%s',(SELECT unitappId FROM gtfw_unit_application WHERE unitappUnitId = '%s' AND unitappApplicationId = '%s'), '%s')";
