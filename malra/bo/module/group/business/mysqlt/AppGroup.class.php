...
//===GET===  
   ...
   function GetDataGroup($groupName, $applicationId, $withPrivilege= false) {
    ...
    function GetDataGroupByIdDetail($groupName, $applicationId, $withPrivilege= false) {

      if ($withPrivilege){
         $result = $this->Open($this->mSqlQueries['get_data_group_with_privilege_by_id_detail'], array('%'.$groupName.'%',$applicationId));

      } else {
         $result = $this->Open($this->mSqlQueries['get_data_group_by_id_detail'], array('%'.$groupName.'%',$applicationId));
      }
      return $result;
   }
   
    function GetDataGuideGroupByIdDetail($groupName, $applicationId) {
      $result = $this->Open($this->mSqlQueries['get_data_guide_group_by_id_detail'], array('%'.$groupName.'%',$applicationId));
      return $result;
   }

   function GetDataGroupAdminUser($groupName, $applicationId, $withPrivilege= false) {

      if ($withPrivilege){
         $result = $this->Open($this->mSqlQueries['get_data_group_with_privilege_admin_user'], array('%'.$groupName.'%',$applicationId));

      } else {
         $result = $this->Open($this->mSqlQueries['get_data_group_admin_user'], array('%'.$groupName.'%',$applicationId));
      }
      return $result;
   }
...
//===DO===
   ...
   function DoAddGroup($applicationId, $groupName, $description, $unitId, $filename) {
      $result = $this->ExecuteInsertQuery($this->mSqlQueries['do_add_group'], array($groupName, $description,$unitId, $applicationId, $filename));
   ...
   function DoUpdateGroup($applicationId, $groupName, $description, $unitId, $filename, $groupId) {
      $result = $this->Execute($this->mSqlQueries['do_update_group'], array($groupName, $description, $unitId, $applicationId, $filename, $groupId));
