...
class PopupInformasi extends Database {
    ...
    function GetListPegawaiNaikPangkatStrategis() {  
		if (!$this->HaveAccess('layanan_pangkat_strategis')) return array();
		...
		$result = $this->Open($this->mSqlQueries['get_list_pegawai_naik_pangkat_strategis'], array($satkerlevel, $satker)); 
		return $result;	  
	}
    
    function GetListPegawaiLayananKgb() {  
		if (!$this->HaveAccess('layanan_sk_kgb')) return array();
		...
		$result = $this->Open($this->mSqlQueries['get_list_pegawai_layanan_kgb'], array($satkerlevel, $satker)); 
		return $result;	  
	}
	...
	function GetCountDaftarPegawaiSatya() {
		...
		if (isset($this->berdasarkan) && $this->berdasarkan =='eselon'){
			if ($this->urutan=='ASC') {
				$this->urutan='DESC';
			}else{
				$this->urutan='ASC';
