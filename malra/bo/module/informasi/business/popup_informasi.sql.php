...
FROM
	...
	LEFT JOIN pub_satuan_kerja ON satkerpegSatkerId=satkerId
WHERE 
	DATEDIFF(pktgolNaikPktYad,NOW()) BETWEEN 0 AND 180
    AND (satkerLevel LIKE CONCAT('%s', '.%%') OR satkerId = '%s')
ORDER BY tanggal
";

$sql['get_list_pegawai_naik_pangkat_strategis'] = "
SELECT DISTINCT
	pegKodeResmi AS nip,
	CONCAT(IFNULL(pegGelarDepan,''),IF(IFNULL(pegGelarDepan,'')='','',' '),pegNama,IF(IFNULL(pegGelarBelakang,'')='','',', '),IFNULL(pegGelarBelakang,'')) AS nama,
	pktgolNaikPktYad AS tanggal,
...
FROM
	...
	LEFT JOIN sdm_satuan_kerja_pegawai ON satkerpegPegId=pegId AND satkerpegAktif='Aktif'
	LEFT JOIN pub_satuan_kerja ON satkerpegSatkerId=satkerId
	LEFT JOIN sdm_sk_pangkat_strategis ON pktgolId=id_pktgol
WHERE 1=1
	AND (pktgolId IS NOT NULL AND id_sk IS NULL)
	AND DATEDIFF(pktgolNaikPktYad,NOW()) BETWEEN 0 AND 180
	AND (satkerLevel LIKE CONCAT('%s', '.%%') OR satkerId = '%s')
ORDER BY tanggal
";

$sql['get_list_pegawai_layanan_kgb'] = "
SELECT DISTINCT
	pegKodeResmi AS nip,
	CONCAT(IFNULL(pegGelarDepan,''),IF(IFNULL(pegGelarDepan,'')='','',' '),pegNama,IF(IFNULL(pegGelarBelakang,'')='','',', '),IFNULL(pegGelarBelakang,'')) AS nama,
...
FROM
	...
	LEFT JOIN sdm_satuan_kerja_pegawai ON satkerpegPegId=pegId AND satkerpegAktif='Aktif'
	LEFT JOIN pub_satuan_kerja ON satkerpegSatkerId=satkerId
	LEFT JOIN sdm_sk_kgb ON kgbId=id_kgb
WHERE 1=1
	AND (kgbId IS NOT NULL AND id_sk IS NULL)
	AND DATEDIFF(kgbTanggalAkanDatang,NOW()) BETWEEN 0 AND 180
	AND (satkerLevel LIKE CONCAT('%s', '.%%') OR satkerId = '%s')
ORDER BY tanggal
";
