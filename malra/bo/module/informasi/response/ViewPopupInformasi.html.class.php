...
class ViewPopupInformasi extends HtmlResponse {
	...
	function TemplateModule() {
		$this->SetTemplateBasedir(GTFWConfiguration::GetValue('application','docroot').'module/informasi/'.GTFWConfiguration::GetValue('application', 'template_address').'');
	...
	function ProcessRequest() {
		...
		$return['naik_pangkat_strategis']=$this->Obj->GetListPegawaiNaikPangkatStrategis();
		$return['layanan_kgb']=$this->Obj->GetListPegawaiLayananKgb();
	...
	function ParseTemplate($data = NULL) {
		...
		$setup_ultah = $this->Setting->GetValueByKey('setup_bday_emp');
        if(strtolower($setup_ultah) == 'yes') {
            $this->mrTemplate->setAttribute('notifikasi_pegawai_ultah', 'visibility', 'visible');
            if (empty($data['ultah'])) {
            ...
            $this->mrTemplate->setAttribute('notifikasi_pegawai_ultah', 'visibility', 'hidden');
        }
		...
		if ($tipe=='ringkasan'){
			...
			$this->mrTemplate->AddVar('tipe', 'TIPE', strtoupper($tipe));
			$val=array('pensiun','naik_pangkat','naik_pangkat_strategis','layanan_kgb','naik_gaji');
			for ($i=0; $i<sizeof($val); $i++){
				if (empty($data[$val[$i]])) {
